...
class FeedbackService:
    ...
    def collect_feedback(self, feedback: Feedback) -> str:
        ...
        try:
            ...
            logger.error(
                "[ERROR] [FEEDBACK] Erro ao coletar feedback: %s",
                str(e),
                exc_info=True
            )
            raise
    ...
    def get_feedback(
        ...
        try:
            ...
            if not memories_list or len(memories_list) == 0:
                logger.debug(
                    "[FEEDBACK] Feedback não encontrado: id=%s",
                    feedback_id
                )
                return None
            ...
            logger.error(
                "[ERROR] [FEEDBACK] Erro ao buscar feedback: %s",
                str(e),
                exc_info=True
            )
            return None
        ...
        try:
            ...
            for memory in memories_list:
                try:
                    ...
                    logger.warning(
                        "[WARN] [FEEDBACK] Erro ao converter memória para Feedback: %s",
                        str(e)
                    )
                    continue
            ...
            logger.error(
                "[ERROR] [FEEDBACK] Erro ao listar feedbacks: %s",
                str(e),
                exc_info=True
            )
            return []
    ...
    def get_feedback_stats(
        ...
        try:
            ...
            logger.error(
                "[ERROR] [FEEDBACK] Erro ao calcular estatísticas: %s",
                str(e),
                exc_info=True
            )
            return {
    ...
    def _memory_to_feedback(self, memory: Dict[str, Any]) -> Feedback:
        ...
        try:
            ...
            if not all([rating, diagnostic_id, user_id, phase]):
                raise ValueError(
                    f"Memória inválida: faltam campos obrigatórios. "
                    f"Metadata: {metadata}"
                )
            ...
            custom_metadata = {
                ...
                if k not in ["feedback_id", "diagnostic_id", "rating", "phase", "user_id", "created_at", "memory_type"]
            }
    ...
    def _sanitize_metadata(self, metadata: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.warning(
            "[WARN] [FEEDBACK] Metadata > 2000 chars (%d). Truncando campos...",
            size
        )
        ...
        if size > 2000:
            ...
            sanitized = {
                ...
                if k in ["feedback_id", "diagnostic_id", "rating", "phase", "user_id", "created_at", "memory_type"]
            }

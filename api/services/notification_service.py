...
class NotificationService:
    ...
    def create_notification(self, notification: Notification) -> str:
        ...
        try:
            ...
            response = self.client.add(
                messages=mem0_content, user_id=notification.user_id, metadata=mem0_metadata
            )
            ...
            logger.error(
                f"[NOTIFICATION] [CREATE] [ERRO] Falha ao criar notificação: {e}", exc_info=True
            )
            raise
    ...
    def mark_as_read(self, notification_id: str) -> bool:
        ...
        try:
            ...
            filters = {"AND": [{"notification_id": notification_id}]}

            results = self.client.search(query="", filters=filters, limit=1)

            if not results or len(results) == 0:
                logger.warning(
                    f"[NOTIFICATION] [READ] Notificação não encontrada | id={notification_id}"
                )
                return False
            ...
            self.client.update(memory_id=mem0_id, data=updated_metadata)
            ...
            logger.error(
                f"[NOTIFICATION] [READ] [ERRO] Falha ao marcar como lida: {e}", exc_info=True
            )
            raise
    ...
    def get_notification(self, notification_id: str) -> Notification | None:
        ...
        try:
            ...
            if not results or len(results) == 0:
                logger.warning(
                    f"[NOTIFICATION] [GET] Notificação não encontrada | id={notification_id}"
                )
                return None
            ...
            notification = Notification(
                ...
                read_at=(
                    datetime.fromisoformat(metadata.get("read_at"))
            ...
            logger.error(
                f"[NOTIFICATION] [GET] [ERRO] Falha ao buscar notificação: {e}", exc_info=True
            )
            return None
    ...
    def list_notifications(
        ...
        try:
            ...
            filters = (
                {"AND": filter_conditions} if filter_conditions else {"AND": [{"user_id": "*"}]}
            )
            ...
            logger.error(
                f"[NOTIFICATION] [LIST] [ERRO] Falha ao listar notificações: {e}", exc_info=True
            )
            return []
    ...
    def get_unread_count(self, user_id: str) -> int:
        ...
        Example:
            ...
            5
        """
        notifications = self.list_notifications(user_id=user_id, status_filter="unread")

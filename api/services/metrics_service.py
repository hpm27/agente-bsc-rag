...
class MetricsService:
    ...
    async def _increment_counter(self, redis_client: aioredis.Redis, key: str, field: str) -> None:
    ...
    async def _record_latency(
        self, redis_client: aioredis.Redis, key: str, latency_ms: float
    ) -> None:
        ...
        if total_samples > 0:
            ...
            await redis_client.setex(key, 7 * 24 * 60 * 60, json.dumps(latency_data))  # TTL 7 dias
    ...
    async def get_top_consumers(self, limit: int = 10, period: str = "24h") -> list[dict[str, Any]]:
            ...
            if len(parts) >= 4:
                ...
                try:
                    ...
                    if key_time >= start_time:
                        ...
                        consumers[api_key]["last_request"] = max(
    ...
    async def get_top_endpoints(
        self, metric: str = "requests", limit: int = 10, period: str = "24h"
    ) -> list[dict[str, Any]]:
        ...
        if metric == "requests":
            ...
            for data in requests_data:
                ...
                if endpoint not in endpoints:
                    endpoints[endpoint] = {"endpoint": endpoint, "requests": 0, "errors": 0}

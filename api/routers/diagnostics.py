...
async def create_diagnostic(
    ...
    """
    logger.info(
        f"[API] create_diagnostic | client_id={body.client_id} | " f"async={body.async_mode}"
    )
    ...
    try:
        ...
        if not body.force_regenerate and "diagnostics" in profile.metadata:
            raise HTTPException(
                400, "Cliente já possui diagnóstico. Use force_regenerate=true para re-gerar."
            )
        ...
        raise HTTPException(500, f"Erro ao criar diagnóstico: {e!s}")
...
async def list_diagnostics(
    ...
    try:
        ...
        profiles_with_diag = [p for p in all_profiles if "diagnostics" in p.metadata]
        ...
        raise HTTPException(500, f"Erro ao listar diagnósticos: {e!s}")
...
async def get_diagnostic(
    ...
    try:
        ...
        raise HTTPException(500, f"Erro ao obter diagnóstico: {e!s}")

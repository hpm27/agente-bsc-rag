...
async def get_traffic(
    ...
    try:
        ...
        if interval not in ["minute", "hour", "day"]:
            raise HTTPException(status_code=400, detail="Intervalo inválido. Use: minute, hour, day")
        ...
        raise HTTPException(status_code=500, detail=f"Erro ao buscar tráfego: {str(e)}")
...
async def get_performance(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Erro ao buscar latência: {str(e)}")
...
async def get_errors(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Erro ao buscar erros: {str(e)}")
...
async def get_consumers(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Erro ao buscar consumers: {str(e)}")
...
async def get_endpoints(
    request: Request,
    response: Response,
    metric: str = Query("requests", description="Métrica para ordenação: requests, latency, errors"),
    ...
    try:
        ...
        if metric not in ["requests", "latency", "errors"]:
            raise HTTPException(status_code=400, detail="Métrica inválida. Use: requests, latency, errors")
        ...
        for ep in endpoints:
            ...
            if metric == "latency":
                results.append(EndpointMetricsResponse(

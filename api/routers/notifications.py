...
async def get_notification(
    ...
    try:
        ...
        if not notification:
            raise HTTPException(
                status_code=404, detail=f"Notificação {notification_id} não encontrada"
            )
...
async def mark_notification_as_read(
    ...
    try:
        ...
        return {"message": "Notificação marcada como lida", "notification_id": notification_id}
...
async def list_notifications(
    ...
    type_filter: str | None = Query(None, description="Filtrar por tipo de evento"),
    status_filter: str | None = Query(None, description="Filtrar por status (unread, read)"),
    priority_filter: str | None = Query(
        None, description="Filtrar por prioridade (high, medium, low)"
    ),
...
async def get_notification_stats(
    ...
    try:
        ...
        logger.error(
            f"[NOTIFICATION] [API] [STATS] Erro ao calcular estatísticas: {e}", exc_info=True
        )

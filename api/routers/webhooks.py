...
from api.schemas.responses import WebhookResponse, WebhookListResponse, WebhookTestResponse
    ...
    try:
        ...
        for webhook_id, webhook_data in _webhooks_storage.items():
            ...
            if "admin" in permissions or webhook_data.get("client_id") == client_id:
                webhooks_list.append(WebhookResponse(
                    webhook_id=webhook_id,
...
async def get_webhook(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Erro ao obter webhook: {str(e)}"
        )
...
async def deactivate_webhook(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Erro ao desativar webhook: {str(e)}"
        )

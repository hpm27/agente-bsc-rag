...
async def get_feedback(
    ...
    user_id: Optional[str] = Query(None, description="ID do usuário (opcional, ajuda na busca)"),
    auth: dict = Depends(verify_api_key)
) -> FeedbackResponse:
    ...
    try:
        ...
        if not feedback:
            raise HTTPException(
                status_code=404,
                detail=f"Feedback não encontrado: {feedback_id}"
            )
...
async def list_feedback(
    request: Request,
    response: Response,
    diagnostic_id: Optional[str] = Query(None, description="Filtrar por diagnóstico"),
    user_id: Optional[str] = Query(None, description="Filtrar por usuário"),
    rating_min: Optional[int] = Query(None, ge=1, le=5, description="Rating mínimo (1-5)"),
    rating_max: Optional[int] = Query(None, ge=1, le=5, description="Rating máximo (1-5)"),
    phase: Optional[str] = Query(None, description="Filtrar por fase do workflow"),
    ...
    if rating_min is not None and rating_max is not None:
        if rating_min > rating_max:
            raise HTTPException(
                status_code=400,
                detail="rating_min não pode ser maior que rating_max"
            )

...
from api.schemas.requests import CreateClientRequest
from api.schemas.responses import ClientListResponse, ClientResponse, ClientSummaryResponse
    ...
    try:
        ...
        profile = ClientProfile(
            ...
            context=StrategicContext(
                current_challenges=body.challenges, strategic_objectives=body.objectives
            ),
        )
        ...
        raise HTTPException(500, f"Erro ao criar cliente: {e!s}")
...
async def list_clients(
    ...
    try:
        ...
        if sector:
            filtered_profiles = [
                p for p in filtered_profiles if p.company.sector.lower() == sector.lower()
            ]
        if phase:
            filtered_profiles = [
                p for p in filtered_profiles if p.engagement.current_phase == phase
            ]
        ...
        return ClientListResponse(
            clients=clients, total=total, page=page, page_size=page_size, has_next=end_idx < total
        )

    except Exception as e:
        logger.error(f"[API] Erro ao listar clientes: {e}", exc_info=True)
        raise HTTPException(500, f"Erro ao listar clientes: {e!s}")
...
async def get_client(
    request: Request, response: Response, client_id: str, auth: dict = Depends(verify_api_key)
):
    ...
    try:
        ...
        raise HTTPException(500, f"Erro ao obter cliente: {e!s}")
...
async def get_client_summary(
    ...
    try:
        ...
        raise HTTPException(500, f"Erro ao obter resumo: {e!s}")

...
llm_model_name_ctx: ContextVar[Optional[str]] = ContextVar("llm_model_name", default=None)
...
class PerformanceMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app, performance_service=None):
        ...
        """
        super().__init__(app)
        self.performance_service = performance_service  # Lazy loading (inicializado no dispatch se None)
    ...
    async def dispatch(self, request: Request, call_next) -> Response:
        ...
        try:
            ...
            logger.error(
                f"[PERFORMANCE] Erro no endpoint {endpoint}: {error_message}",
                exc_info=True
            )
    ...
    async def _extract_diagnostic_id(self, response: Response) -> Optional[str]:
        ...
        try:
            ...
            # Recriar body_iterator para response não ficar corrompida
            response.body_iterator = (chunk for chunk in [body])
            ...
            diagnostic_id = (
                data.get("diagnostic_id") or
                data.get("id") or
                data.get("result", {}).get("diagnostic_id")
            )

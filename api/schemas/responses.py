...
class ReportResponse(BaseModel):
    ...
    expires_at: str | None = Field(
        default=None, description="Data de expiração do download (7 dias típico)"
    )
...
class FeedbackRequest(BaseModel):
    ...
    comment: str | None = Field(None, max_length=1000, description="Feedback textual opcional")
...
class FeedbackResponse(BaseModel):
    ...
    comment: str | None = Field(None, description="Feedback textual")
...
class NotificationRequest(BaseModel):
    ...
    type: str = Field(
        description="Tipo de evento (diagnostic_completed, refinement_completed, feedback_received, error_occurred)"
    )
...
class NotificationResponse(BaseModel):
    ...
    diagnostic_id: str | None = Field(None, description="ID do diagnóstico")
    ...
    read_at: str | None = Field(
        None, description="Timestamp ISO 8601 de leitura (None se não lida)"
    )
...
class MetricsListResponse(BaseModel):
    ...
    metrics: list[dict[str, Any]] = Field(
        description="Lista de métricas (PerformanceMetrics serializadas)"
    )
...
class MetricsStatsResponse(BaseModel):
    ...
    latency: dict[str, float] = Field(
        description="Latências (p50_ms, p95_ms, mean_ms, min_ms, max_ms)"
    )

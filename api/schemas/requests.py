...
class CreateClientRequest(BaseModel):
    ...
    size: Literal["pequena", "média", "grande"] | None = Field(
        default=None, description="Porte da empresa (opcional)"
    )
    revenue: str | None = Field(
        default=None, description="Faixa de receita (opcional)", examples=["R$ 1-10M", "R$ 10-50M"]
    )
...
class CreateDiagnosticRequest(BaseModel):
    ...
    client_id: str = Field(description="ID do cliente para diagnosticar", examples=["cli_abc123"])
    async_mode: bool = Field(
        default=False, description="Se True, retorna imediatamente e envia resultado via webhook"
    )
    ...
    force_regenerate: bool = Field(
        default=False, description="Se True, força re-gerar mesmo que já exista diagnóstico"
    )
...
class ToolExecutionRequest(BaseModel):
    ...
    client_id: str = Field(description="ID do cliente", examples=["cli_abc123"])
    context_override: dict | None = Field(
        default=None, description="Contexto customizado (sobrescreve dados do cliente)"
    )
    async_mode: bool = Field(
        default=False, description="Execução assíncrona (webhook quando completo)"
    )
...
class KpiDefinitionRequest(ToolExecutionRequest):
    ...
    perspective: Literal["financial", "customer", "process", "learning"] | None = Field(
        default=None, description="Perspectiva BSC específica (None = todas)"
    )
...
class GeneratePdfRequest(BaseModel):
    ...
    diagnostic_id: str | None = Field(
        default=None, description="ID do diagnóstico (obrigatório para PDF diagnóstico)"
    )
    client_id: str | None = Field(
        default=None, description="ID do cliente (alternativa a diagnostic_id)"
    )
    report_type: Literal["diagnostic_full", "diagnostic_perspective", "executive_summary"] = Field(
        default="diagnostic_full", description="Tipo de relatório PDF"
    )
    perspective: Literal["financial", "customer", "process", "learning"] | None = Field(
        default=None,
    ...
    format_style: Literal["professional", "compact"] = Field(
        default="professional", description="Estilo de formatação"
    )

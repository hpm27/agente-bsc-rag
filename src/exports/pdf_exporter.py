...
class PdfExporter:
    ...
    def export_full_diagnostic(
        ...
        """
        logger.info(
            f"[PDF_EXPORT] Exportando diagnóstico completo | " f"company={profile.company.name}"
        )
        ...
        # Renderizar HTML
        html_content = self.template_manager.render_full_diagnostic(diagnostic, profile)
    ...
    def export_perspective(
        ...
        # Renderizar HTML
        html_content = self.template_manager.render_perspective(diagnostic, profile, perspective)
    ...
    def _html_to_pdf(self, html_content: str, output_path: Path) -> None:
        ...
        try:
            ...
            # Gerar PDF
            html_doc.write_pdf(target=str(output_path), font_config=self.font_config)

        except ImportError:
            raise  # Re-raise ImportError
        except Exception as e:
            logger.error(f"[PDF_EXPORT] Erro ao gerar PDF: {e}")
            raise OSError(f"Erro ao gerar PDF: {e}") from e
    ...
    def _validate_complete_diagnostic(self, diagnostic: CompleteDiagnostic) -> None:
        ...
        if missing:
            raise ValueError(f"Diagnóstico incompleto. Perspectivas faltando: {', '.join(missing)}")

...
from src.memory.schemas import ClientProfile, CompleteDiagnostic, Recommendation
...
class CsvExporter:
    ...
    def export_clients_list(
        ...
        for profile in profiles:
            row = {
                ...
                "approval_status": profile.metadata.get("approval_status", "N/A") if profile.metadata else "N/A",  # approval_status está em metadata
                "created_at": profile.created_at.strftime("%d/%m/%Y %H:%M") if profile.created_at else "N/A",
                "updated_at": profile.updated_at.strftime("%d/%m/%Y %H:%M") if profile.updated_at else "N/A",
                "total_challenges": len(profile.context.current_challenges) if profile.context and profile.context.current_challenges else 0,
                "total_objectives": len(profile.context.strategic_objectives) if profile.context and profile.context.strategic_objectives else 0,
            }
            data.append(row)
    ...
    def export_recommendations(
        ...
        for i, rec in enumerate(diagnostic.recommendations, 1):
            row = {
                ...
                "expected_outcomes": " | ".join(rec.expected_outcomes) if rec.expected_outcomes else "N/A",
            }
            data.append(row)
    ...
    def export_perspectives_summary(
        ...
        """
        logger.info(
            f"[CSV_EXPORT] Exportando resumo perspectivas | "
        ...
        for name, persp in perspectives:
            row = {
                ...
                "current_state": persp.current_state[:200] + "..." if len(persp.current_state) > 200 else persp.current_state,
            }
            data.append(row)

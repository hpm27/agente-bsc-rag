...
class ConsultingPhase(str, Enum):
    ...
    - Alignment: Causa-efeito entre perspectivas
    
    Transições:
    - → IMPLEMENTATION: Solução aprovada
    - → APPROVAL_PENDING: Revisar solução
    """
    ...
    Futuro (Fase 8-9):
    - Action plans com milestones
    - MCPs: Asana, Google Calendar integrations
...
class ConsultingState(TypedDict, total=False):
    ...
    previous_phase: Optional[ConsultingPhase]
...
def should_transition(
    ...
    if current_phase == ConsultingPhase.ONBOARDING and target_phase == ConsultingPhase.DISCOVERY:
        ...
        return (
            profile.get("company") is not None and
            profile.get("strategic_context") is not None
        )
    
    # DISCOVERY → APPROVAL_PENDING: Pelo menos 1 ferramenta executada
    if current_phase == ConsultingPhase.DISCOVERY and target_phase == ConsultingPhase.APPROVAL_PENDING:
        tool_outputs = state.get("tool_outputs", {})
        return len(tool_outputs) > 0
    
    # APPROVAL_PENDING → DISCOVERY: Refinamento solicitado
    if current_phase == ConsultingPhase.APPROVAL_PENDING and target_phase == ConsultingPhase.DISCOVERY:
        return state.get("approval_status") == ApprovalStatus.REJECTED
    
    # APPROVAL_PENDING → SOLUTION_DESIGN: Aprovado (Fase futura)
    if current_phase == ConsultingPhase.APPROVAL_PENDING and target_phase == ConsultingPhase.SOLUTION_DESIGN:

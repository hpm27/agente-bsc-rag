...
class JudgmentResult(BaseModel):
    ...
    quality_score: float = Field(description="Score de qualidade de 0 a 1", ge=0.0, le=1.0)
    is_complete: bool = Field(description="Se a resposta está completa")
    has_sources: bool = Field(description="Se a resposta cita fontes apropriadamente")
    is_grounded: bool = Field(description="Se a resposta está fundamentada nos documentos")
    issues: list[str] = Field(description="Lista de problemas encontrados", default_factory=list)
    suggestions: list[str] = Field(description="Sugestões de melhoria", default_factory=list)
    verdict: str = Field(
...
Seja equilibrado: respostas de alta qualidade (>= 0.85) podem ser aprovadas mesmo com fundamentação imperfeita, mas respostas mediocres precisam boa fundamentação."""
    ...
    def evaluate(
        ...
        try:
            ...
            return JudgmentResult(
                ...
                has_sources=False,
                is_grounded=False,
                issues=[f"Erro na avaliação: {e!s}"],
                suggestions=["Reavalie manualmente a resposta"],
                verdict="needs_improvement",
                reasoning=f"Falha no processo de avaliação: {e!s}",
            )
    ...
    def evaluate_multiple(
        ...
        for agent_resp in agent_responses:
            ...
            results.append({**agent_resp, "judgment": judgment})

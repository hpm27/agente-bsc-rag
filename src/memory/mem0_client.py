...
class Mem0ClientWrapper:
    ...
    def _serialize_profile(self, profile: ClientProfile) -> dict:
        ...
        try:
            ...
            logger.debug("[OK] Profile %r serializado (%d campos)", profile.client_id, len(data))
            return data
        except Exception as e:
        ...
        try:
            ...
            if not isinstance(data, dict):
                raise ProfileValidationError(
                    user_id, ValueError("profile_data inválido: esperado dict")
                )
            company = data.get("company")
            if not isinstance(company, dict):
                raise ProfileValidationError(
                    user_id, ValueError("profile_data.company ausente ou inválido")
                )
    ...
    def save_profile(self, profile: ClientProfile) -> str:
        ...
        try:
            ...
            challenges_text = (
                ", ".join(profile.context.current_challenges)
                if profile.context.current_challenges
                else "ainda não identificados"
            )
            objectives_text = (
                ", ".join(profile.context.strategic_objectives)
                if profile.context.strategic_objectives
                else "em definição"
            )
            ...
            try:
                ...
                # [TIMER] CRÍTICO: Aguardar delete_all completar (eventual consistency)
            ...
            if metadata_size > 2000:
                ...
                if metadata_size > 2000:
                    ...
                    metadata_size = _calculate_metadata_size(metadata_compact)
                    logger.warning(
                        "[WARN] Metadata reduzido para campos mínimos (size: %d)", metadata_size
                    )

            logger.debug("[OK] Metadata sanitizado: %d chars (limit: 2000)", metadata_size)

            # Agora salva no Mem0 usando user_id como chave
            self.client.add(messages=messages, user_id=profile.client_id, metadata=metadata_compact)

            # [TIMER] CRÍTICO: Aguardar add() completar (eventual consistency)
    ...
    def update_profile(self, user_id: str, updates: dict[str, Any]) -> ClientProfile:
    ...
    def list_all_profiles(self, limit: int = 100, include_archived: bool = False) -> list:
        ...
        try:
            ...
            try:
                ...
                logger.debug(
                    "[OK] get_all(filters=user_id:*) retornou %d resultados",
            ...
            if not results:
                try:
                    results = self.client.search(
                        ...
                        filters={"AND": [{"user_id": "*"}]},  # [OK] OBRIGATÓRIO na API v2!
                        limit=limit,
                    )
                    logger.debug(
                        "[OK] search(query='empresa', filters=*) retornou %d resultados",
                        len(results) if results else 0,
                    )
                except Exception as e:
                    last_error = e
            ...
            if not results:
                try:
                    ...
                    logger.debug(
                        "[OK] search(query='', filters=*) retornou %d resultados",
                        len(results) if results else 0,
                    )
                except Exception as e:
                    last_error = e
                ...
                for memory in (all_memories if isinstance(all_memories, list) else [all_memories]):
                    ...
                    if "benchmark_report_data" in metadata:
                        ...
                        memory_id = (
                            ...
                            else getattr(memory, "id", None)
                        )
                        if memory_id:
                            self.client.delete(memory_id=memory_id)
                            logger.debug(
                                "[CLEANUP] Benchmark antigo deletado (client_id=%r)", client_id
                            )
            except Exception as delete_error:
                logger.debug(
            ...
            for memory in (memories if isinstance(memories, list) else [memories]):
                try:
                    ...
                    if "benchmark_report_data" in metadata:
                        ...
                        logger.info("[OK] Benchmark report carregado para client_id=%r", client_id)
                        return report_data

                except Exception as e:
                    logger.warning("[WARN] Erro ao processar memória (ignorando): %s", e)
                    continue

            # Nenhum benchmark encontrado
            logger.debug("[INFO] Nenhum benchmark report encontrado para client_id=%r", client_id)
            return None
    ...
    def save_tool_output(self, client_id: str, tool_output: Any) -> str:
        ...
        try:
            ...
            if not hasattr(tool_output, "tool_name") or not hasattr(
            ...
            tool_data = (
                tool_output.tool_output_data
                if not isinstance(tool_output.tool_output_data, dict)
                else tool_output.tool_output_data
            )
            ...
            try:
                ...
                for memory in (all_memories if isinstance(all_memories, list) else [all_memories]):
                    try:
                        ...
                        logger.debug("[CLEANUP] Erro ao processar memória (ignorando): %s", e)
                        continue
            except Exception as delete_error:
                logger.debug(
            ...
            self.client.add(
                ...
                metadata={
                    ...
                    "created_at": (
                        tool_output.created_at.isoformat()
                        if hasattr(tool_output.created_at, "isoformat")
                        else str(tool_output.created_at)
                    ),
                },
            )

...
def create_vector_store(
    store_type: VectorStoreType = None, embedding_dim: int = 3072, **kwargs
) -> BaseVectorStore:
    """
    ...
    if store_type == "qdrant":
        return QdrantVectorStore(
            ...
            default_index_name=kwargs.get(
                "index_name", getattr(settings, "vector_store_index", "bsc_documents")
            ),
        )
    ...
    if store_type == "weaviate":
        return WeaviateVectorStore(
            ...
            embedding_dim=embedding_dim,
            default_index_name=kwargs.get(
                "index_name", getattr(settings, "vector_store_index", "BscDocuments")
            ),
        )
...
def compare_vector_stores() -> dict:
    ...
    return {
        "qdrant": {
            ...
            "cons": ["Hybrid search requer integração externa", "BM25 não nativo"],
        },
        "weaviate": {
            "performance": 8,
        ...
        "redis": {
            ...
            "pros": ["Muito rápido", "Familiaridade", "Versátil (cache + vector)", "Fácil setup"],
            "cons": [
                "Menos features RAG",

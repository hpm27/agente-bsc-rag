...
class QueryDecomposer:
    ...
    def __init__(
        ...
        self.prompt_template = PromptTemplate(
            template=QUERY_DECOMPOSITION_PROMPT, input_variables=["query"]
        )
    ...
    def _calculate_complexity_score(self, query: str) -> int:
        ...
        perspectives_count = sum(
            1 for perspective in self.BSC_PERSPECTIVES if perspective in query_lower
        )
        ...
        multiple_perspectives_pattern = any(
            re.search(pattern, query_lower) for pattern in perspective_patterns
        )

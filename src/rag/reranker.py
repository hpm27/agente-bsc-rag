...
class CohereReranker:
    ...
    def _detect_language(self, text: str) -> Literal["pt-br", "en", "other"]:
        ...
        # Decisão
        if has_pt_accents or (pt_count >= 1 and en_count == 0):
            return "pt-br"
    ...
    def rerank(
        ...
        if adaptive_multilingual and query_lang == "pt-br":
            ...
            logger.debug(
                f"[ADAPTIVE] Query PT-BR detectada, ajustando top_n: {top_n} -> {adjusted_top_n}"
            )
        else:
        ...
        try:
            ...
            results = self.client.rerank(
                model=self.model, query=query, documents=texts, top_n=adjusted_top_n
            )
    ...
    def rerank_with_scores(
        ...
        logger.debug(
            f"Filtrados {len(filtered)}/{len(reranked)} documentos (threshold={threshold})"
        )
        return filtered

    def _calculate_similarity(self, embeddings_matrix: np.ndarray) -> np.ndarray:
        """
    ...
    def calculate_adaptive_topn(self, query: str, base_top_n: int = 10) -> int:
        """
    ...
    def rerank_with_diversity(
        ...
        if embeddings.shape[0] != len(documents):
            logger.error(
                f"[MMR] Mismatch: {embeddings.shape[0]} embeddings vs {len(documents)} docs"
            )
        ...
        diversity_threshold = (
            diversity_threshold if diversity_threshold is not None else settings.diversity_threshold
        )
        ...
        relevance_scores = np.array([doc.get("rerank_score", 0.5) for doc in documents])
        ...
        while len(selected_indices) < top_n and remaining_indices:
            ...
            if enable_metadata_boost:
                ...
                metadata_boosts = dict.fromkeys(remaining_indices, 1.0)
            ...
            for idx in remaining_indices:
                ...
                if not selected_indices:
                    ...
                    mmr_score = (
                        lambda_param * boosted_relevance - (1 - lambda_param) * max_similarity
                    )
        ...
        logger.info(
            f"[MMR] Re-rankeados {len(diverse_docs)} documentos com diversidade (λ={lambda_param:.2f})"
        )
        return diverse_docs
...
class FusionReranker:
    ...
    def fuse(
        self, results_list: list[list[dict[str, Any]]], top_n: int = None
    ) -> list[dict[str, Any]]:
        """
        ...
        for results in results_list:
            for rank, doc in enumerate(results, start=1):
                ...
                if key in doc_scores:
                    ...
                    doc_scores[key] = {**doc, "rrf_score": rrf_score}
...
class HybridReranker:
    ...
    def rerank(
        ...
        fused = self.fusion_reranker.fuse(
            [vector_results, text_results], top_n=top_n * 2  # Pega mais para re-rankear
        )

...
class RedisVectorStore(BaseVectorStore):
    ...
    def hybrid_search(
        ...
        vector_results = self.vector_search(
            query_embedding, k=k * 2, filter_dict=filter_dict, index_name=index_name
        )

        # Busca por texto
        text_results = self.text_search(
            query, k=k * 2, filter_dict=filter_dict, index_name=index_name
        )
...
def create_redis_vector_store(
    host: str = None, port: int = None, password: str = None, embedding_dim: int = 3072
) -> RedisVectorStore:
    """

...
class QdrantVectorStore(BaseVectorStore):
        ...
        **kwargs,
        ...
        if force_recreate:
            ...
            try:
                ...
                logger.info(
                    f"Coleção {index_name} já existe. Use force_recreate=True para recriar."
                )
                return
            except:
    ...
    def vector_search(
        ...
        results = self.client.query_points(
            collection_name=index_name, query=query_vector, limit=k, query_filter=search_filter
        )
    ...
    def hybrid_search(
        ...
        results = self.vector_search(
            query_embedding=query_embedding, k=k, filter_dict=filter_dict, index_name=index_name
        )
    ...
    def delete_documents(self, document_ids: list[str], index_name: str | None = None):
        ...
        self.client.delete(collection_name=index_name, points_selector=point_ids)
    ...
    def get_stats(self, index_name: str | None = None) -> VectorStoreStats:
        ...
        try:
            ...
            return VectorStoreStats(
                ...
                additional_info={"indexed_vectors_count": indexed_count},
            )
        except Exception as e:
    ...
    def _build_filter(self, filter_dict: dict[str, Any]) -> Filter:
        ...
        for key, value in filter_dict.items():
            ...
            if isinstance(value, dict) and "$in" in value:
                ...
                conditions.append(FieldCondition(key=key, match=MatchAny(any=values_list)))
            else:
    ...
    def _convert_results(self, results: list[ScoredPoint], search_type: str) -> list[SearchResult]:
        """
        ...
        for point in results:
            ...
            search_results.append(
                SearchResult(
                    ...
                    metadata={
                        k: v for k, v in payload.items() if k not in ["content", "source", "page"]
                    },
                )
            )

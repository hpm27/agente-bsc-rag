...
class AlignmentValidatorTool:
    ...
    def validate_strategy_map(self, strategy_map: StrategyMap) -> AlignmentReport:
        ...
        logger.info(
            f"[AlignmentValidator] Validação completa: score={score:.1f}%, "
        ...
        for name, perspective in perspectives.items():
            ...
            if obj_count < 2:
                ...
                recommendations.append(
                    f"Adicionar pelo menos {2 - obj_count} objetivo(s) em '{name}'"
                )

            elif obj_count > 10:
                passou = False
                gaps.append(
                    f"Perspectiva '{name}' tem {obj_count} objectives (máximo 10 - RED FLAG: falta cascading)"
                )
                recommendations.append(
                    f"Revisar objectives em '{name}' e fazer cascading para nível tático"
                )
    ...
    def _check_cause_effect_exists(
        ...
        if conn_count < 4:
            ...
            recommendations.append(
                f"Adicionar pelo menos {4 - conn_count} conexões entre perspectivas"
            )
    ...
    def _check_no_isolated_objectives(
        ...
        for isolated_name in isolated_obj_names:
            warnings.append(
                f"Objective '{isolated_name}' está isolated (sem conexões causa-efeito)"
            )
            ...
            for obj in perspective.objectives:
                for kpi_name in obj.related_kpis:
                    ...
                    if not is_measurable:
                        passou = False
                        warnings.append(
                            f"KPI '{kpi_name}' parece não ser mensurável (falta número/unidade)"
                        )
                        recommendations.append(
                            f"Tornar KPI '{kpi_name}' SMART (ex: adicionar meta numérica)"
                        )
            ...
            for obj in perspective.objectives:
                ...
                for keyword in OPERATIONAL_KEYWORDS:
                    if keyword in text:
                        passou = False
                        warnings.append(
                            f"Objective '{obj.name}' parece operacional (contém '{keyword}')"
                        )
                        recommendations.append(
                            f"Reformular '{obj.name}' como goal estratégico (onde chegar, não como fazer)"
                        )
        ...
        for perspective in all_perspectives:
            for obj in perspective.objectives:
                ...
                if desc_length < 50:
                    passou = False
                    warnings.append(
                        f"Objective '{obj.name}' tem description muito curta ({desc_length} chars, mínimo 50)"
                    )
                    recommendations.append(
                        f"Expandir rationale de '{obj.name}' explicando WHY escolhemos esse objective"
                    )
            ...
            for obj in perspective.objectives:
                ...
                for jargon in GENERIC_JARGON:
                    if jargon in text:
                        ...
                        recommendations.append(
                            f"Substituir jargon em '{obj.name}' por linguagem específica e mensurável"
                        )
                        break  # Evitar múltiplos warnings para mesmo objective

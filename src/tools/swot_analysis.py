...
from src.memory.schemas import CompanyInfo, StrategicContext, SWOTAnalysis
...
class SWOTAnalysisTool:
    ...
    async def facilitate_swot(
        ...
        """
        logger.info(
            f"[SWOT Tool] Facilitando SWOT para {company_info.name} " f"(use_rag={use_rag})"
        )
        ...
        bsc_knowledge = ""
        if use_rag:
            bsc_knowledge = await self._retrieve_bsc_knowledge(company_info, strategic_context)
            logger.info(f"[SWOT Tool] Conhecimento BSC recuperado " f"({len(bsc_knowledge)} chars)")
        else:
        ...
        try:
            ...
            if not swot.is_complete(min_items_per_quadrant=2):
                logger.warning(f"[SWOT Tool] SWOT incompleto! Resumo: {swot.quadrant_summary()}")
        ...
        try:
            ...
            for agent_name, result in zip(agent_names, results):
                if isinstance(result, Exception):
                    logger.error(f"[SWOT Tool] {agent_name} agent falhou: {result}")
                    continue
                ...
                if isinstance(result, dict):
                    ...
                    if context_str and len(context_str) > 50:  # Filtrar contexts vazios
                        ...
                        logger.debug(f"[SWOT Tool] {agent_name} agent: {len(context_str)} chars")

...
class IterationOutput(BaseModel):
    ...
    confidence: float = Field(
        ge=0.0,
        le=1.0,
        description="Confianca de que esta resposta e relevante"
    )
    is_root_cause: bool = Field(
        default=False,
        description="True se esta resposta e a causa raiz fundamental"
    )
    reasoning: str = Field(
        min_length=20,
        description="Breve explicacao do raciocinio"
    )
...
class RootCauseOutput(BaseModel):
    ...
    reasoning: str = Field(
        min_length=50,
        description="Explicacao de como chegamos a esta causa"
    )
    recommended_actions: list[str] = Field(
        min_length=2,
        description="Acoes concretas para resolver causa raiz"
    )
...
class FiveWhysTool:
    ...
    def facilitate_five_whys(
        ...
        if use_rag and self._rag_available():
            ...
            logger.info(
                f"[Five Whys Tool] Conhecimento BSC recuperado "
        ...
        if len(iterations) < 3:
            raise ValueError(
                f"5 Whys requer minimo 3 iteracoes (coletadas: {len(iterations)})"
            )
        
        logger.info(
            f"[Five Whys Tool] Loop iterativo completo: {len(iterations)} iteracoes"
        )
    ...
    def _synthesize_root_cause(
        ...
        """
        logger.debug(
            f"[Five Whys Tool] Sintetizando root cause de {len(iterations)} iteracoes"
        )
    ...
    def _retrieve_bsc_knowledge(
        ...
        try:
            ...
            logger.error(
                f"[Five Whys Tool] Erro ao recuperar conhecimento BSC via RAG: {e}"
            )

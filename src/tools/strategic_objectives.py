...
class StrategicObjectivesTool:
    ...
    def __init__(
        ...
        llm: BaseLLM,
        use_rag: bool = False,
        rag_agents: Optional[tuple] = None
    ):
    ...
    def _define_perspective_objectives(
        ...
        Raises:
            RuntimeError: Se LLM falha ao gerar objetivos
        """
        logger.info(f"[StrategicObjectivesTool] Definindo objetivos para perspectiva: {perspective}")
        ...
        try:
            ...
            objectives_data = result["objectives"] if isinstance(result, dict) else result.objectives
            
            # Converter para StrategicObjective Pydantic
            objectives = []
            for obj_item in objectives_data:
                # Converter para dict se necessario
                obj_dict = obj_item if isinstance(obj_item, dict) else obj_item.dict() if hasattr(obj_item, 'dict') else dict(obj_item)
            ...
            raise RuntimeError(
                f"Falha ao gerar objetivos para perspectiva {perspective}: {str(e)}"
            ) from e
    ...
    def _retrieve_bsc_knowledge(
        ...
        logger.info(
            f"[StrategicObjectivesTool] Buscando conhecimento BSC para {perspective}"
        )
        ...
        try:
            ...
            structured_llm = self.llm.with_structured_output(
                schema={
                    ...
                    "required": [
                        "is_balanced", "balance_analysis", "alignment_score",
                        "alignment_analysis", "overall_quality"
                    ]
                }
            )

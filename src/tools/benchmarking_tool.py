...
class BenchmarkingTool:
    ...
    def __init__(
        self,
        llm: BaseLLM,
        retriever: BSCRetriever | None = None,
        max_retries: int = 3
    ):
    ...
    def generate_benchmarks(
        ...
        if diagnostic is None or len(diagnostic) == 0:
            raise ValueError(
                "diagnostic obrigatório para benchmarking. "
                "Execute diagnóstico BSC primeiro."
            )
        ...
        if use_rag and self.retriever is not None:
            ...
            try:
                ...
                logger.info(
                    "[BenchmarkingTool] RAG context retrieved (%d docs)",
                    len(rag_docs)
                )
            except Exception as e:
                logger.warning(
                    "[BenchmarkingTool] RAG retrieval falhou (%s). Continuando sem RAG.",
                    str(e)
                )
                rag_docs = None
        ...
        for attempt in range(1, self.max_retries + 1):
            try:
                logger.info(
                    "[BenchmarkingTool] Invoking LLM (attempt %d/%d)...",
                    attempt,
                    self.max_retries
                )
    ...
    def _validate_report_quality(
        ...
        logger.info(
            ...
            "none" if len(positive_gaps) != len(report.comparisons) and len(negative_gaps) != len(report.comparisons) else "viés detectado"
        )

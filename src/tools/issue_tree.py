...
class SubProblemOutput(BaseModel):
    ...
    text: str = Field(min_length=5, max_length=300, description="Descricao do sub-problema")
    category: str | None = Field(
        default=None,
        description="Perspectiva BSC (Financeira, Clientes, Processos, Aprendizado) ou None",
    )
    is_leaf: bool = Field(
        default=False, description="True se sub-problema e folha (nao precisa decompor mais)"
    )
    reasoning: str = Field(min_length=20, description="Breve explicacao da decomposicao MECE")
...
class DecompositionOutput(BaseModel):
    ...
    sub_problems: list[SubProblemOutput] = Field(
        min_length=2, max_length=4, description="Sub-problemas identificados (2-4 para MECE)"
    )
...
class SolutionPathsOutput(BaseModel):
    ...
    reasoning: str = Field(
        min_length=50, description="Explicacao de como leaf nodes foram sintetizados"
    )
...
class IssueTreeTool:
    ...
    async def facilitate_issue_tree(
        self,
        ...
        """
        logger.info(
            f"[Issue Tree] Iniciando analise: root_problem='{root_problem[:50]}...', max_depth={max_depth}"
        )

        # Validacoes
        if not root_problem or len(root_problem.strip()) < 10:
            raise ValueError(
                f"root_problem deve ter minimo 10 caracteres (recebido: {len(root_problem)})"
            )
        ...
        nodes = [root_node]
        logger.info(
            f"[Issue Tree] Root node criado: id={root_node.id}, text='{root_node.text[:50]}...'"
        )
        ...
        if use_rag:
            rag_context = await self._retrieve_bsc_knowledge(
                company_info, strategic_context, root_problem
            )
        ...
        while current_level < max_depth:
            ...
            if not nodes_to_decompose:
                logger.info(
                    f"[Issue Tree] Decomposicao completa no nivel {current_level} (sem nodes para decompor)"
                )
                break

            logger.info(
                f"[Issue Tree] Decomposicao nivel {current_level}: {len(nodes_to_decompose)} nodes"
            )
            ...
            for parent_node in nodes_to_decompose:
                try:
                    ...
                    logger.info(
                        f"[Issue Tree] Node decomposto: parent='{parent_node.text[:30]}...', {len(children_nodes)} children criados"
                    )

                except Exception as e:
                    # Se decomposicao falha, marcar parent como leaf (fallback)
                    logger.warning(
                        f"[Issue Tree] Falha ao decompor node '{parent_node.text[:30]}...': {e}"
                    )
        ...
        logger.info(
            f"[Issue Tree] Arvore construida: {len(nodes)} nodes, {actual_max_depth} niveis profundidade"
        )
    ...
    def _decompose_node(
        ...
        try:
            ...
            if not (2 <= len(decomposition.sub_problems) <= 4):
                raise ValueError(
                    f"Decomposicao deve ter 2-4 sub-problemas (recebido: {len(decomposition.sub_problems)})"
                )
            ...
            for sub_prob in decomposition.sub_problems:
                child_node = IssueNode(
                    ...
                    is_leaf=sub_prob.is_leaf
                    or current_level >= max_depth,  # Leaf se LLM disse OU max depth
                    category=sub_prob.category,
                )
                children_nodes.append(child_node)

            logger.debug(
                f"[Issue Tree] Decomposicao LLM: {len(children_nodes)} sub-problemas, MECE validation: {decomposition.mece_validation[:50]}..."
            )
    ...
    async def _retrieve_bsc_knowledge(
        self, company_info: CompanyInfo, strategic_context: StrategicContext, root_problem: str
    ) -> str:
        """Recupera conhecimento BSC via RAG (4 specialist agents) EM PARALELO.
...
Buscar conhecimento BSC (Balanced Scorecard Kaplan & Norton) sobre decomposicao de problemas estrategicos similares, frameworks de analise, e recomendacoes para as 4 perspectivas BSC."""
            ...
            # SPRINT 1 OTIMIZAÇÃO: Executar RAG nos 4 specialist agents EM PARALELO
            ...
            # Processar resultados e extrair conhecimento
            knowledge_parts = []
            agent_info = [
                ("Financial", "Perspectiva Financeira BSC"),
                ("Customer", "Perspectiva Clientes BSC"),
                ("Process", "Perspectiva Processos Internos BSC"),
                ("Learning", "Perspectiva Aprendizado e Crescimento BSC"),
            ]

            for (agent_name, perspective_label), result in zip(agent_info, results):
                if isinstance(result, Exception):
                    logger.warning(f"[Issue Tree RAG] {agent_name} agent falhou: {result}")
                    continue

                if isinstance(result, dict):
                    # Tentar 'response' primeiro (formato esperado), depois 'output' (formato atual)
                    response_text = result.get("response") or result.get("output", "")

                    if response_text:
                        knowledge_parts.append(f"[{perspective_label}]\n{response_text}")

            # Consolidar conhecimento
            if knowledge_parts:
                consolidated = "\n\n".join(knowledge_parts)
                logger.info(
                    f"[Issue Tree RAG] Conhecimento consolidado: {len(knowledge_parts)} perspectivas, {len(consolidated)} chars"
                )
                return consolidated
            logger.warning(
                "[Issue Tree RAG] Nenhum conhecimento recuperado (todos agents falharam)"
            )
            return ""
    ...
    def _synthesize_solution_paths(
        ...
        try:
            ...
            if not leaf_nodes:
                logger.warning(
                    "[Issue Tree Synthesis] Nenhum leaf node identificado (arvore incompleta)"
                )
                return []
            ...
            logger.info(
                f"[Issue Tree Synthesis] {len(synthesis.solution_paths)} solution paths gerados"
            )

...
class KPIDefinerTool:
    ...
    def __init__(
        self,
        llm: BaseLLM,
        financial_agent: SpecialistAgent | None = None,
        customer_agent: SpecialistAgent | None = None,
        process_agent: SpecialistAgent | None = None,
        learning_agent: SpecialistAgent | None = None,
    ):
        ...
        """
        self.llm = llm
        self.financial_agent: SpecialistAgent | None = financial_agent
        self.customer_agent: SpecialistAgent | None = customer_agent
        self.process_agent: SpecialistAgent | None = process_agent
        self.learning_agent: SpecialistAgent | None = learning_agent
    ...
    def define_kpis(
        ...
        if not strategic_context:
            raise ValueError(
                "strategic_context ausente. " "Execute onboarding completo antes de definir KPIs."
            )
        ...
        logger.info(
            f"[KPI Definer Tool] Definindo KPIs para {company_info.name} " f"(use_rag={use_rag})"
        )
        ...
        for perspective in perspectives:
            ...
            logger.info(f"[KPI Definer Tool] {len(kpis)} KPIs definidos para {perspective}")
    ...
    def _define_perspective_kpis(
        ...
        try:
            ...
            raise ValueError(f"Falha ao gerar KPIs para perspectiva {perspective}: {e}") from e

    def _retrieve_bsc_knowledge(self, perspective: str, context: str) -> list[dict]:
        ...
        if not self._has_rag_agents():
            logger.warning(
                "[KPI Definer Tool] Specialist agents nao configurados, RAG desabilitado"
            )
            return []
    ...
    def _has_rag_agents(self) -> bool:
        ...
        """
        return all(
            [self.financial_agent, self.customer_agent, self.process_agent, self.learning_agent]
        )
    ...
    def _validate_kpi_balance(self, framework: KPIFramework) -> bool:
        ...
        if total > 32:
            logger.warning(
                f"[KPI Definer Tool] Framework tem {total} KPIs total, " f"recomendado maximo 32"
            )
            return False

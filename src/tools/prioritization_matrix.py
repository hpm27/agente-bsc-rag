...
class PrioritizationMatrixTool:
    ...
    async def prioritize(
        ...
        try:
            ...
            logger.info(f"PrioritizationMatrix criada com sucesso: {matrix.total_items} items priorizados")
            logger.info(f"Distribuição prioridades: {matrix.critical_count} CRITICAL, {matrix.high_count} HIGH, "
    ...
    async def _call_llm_with_retry(
        ...
        prompt: str,
        prioritization_context: str,
        weights_config: Optional[dict[str, float]],
        max_retries: int = 3
    ) -> PrioritizationMatrix:
        ...
        for attempt in range(max_retries):
            try:
                logger.debug(f"[PrioritizationMatrixTool] Tentativa {attempt + 1}/{max_retries} - Chamando LLM structured output")
                ...
                if matrix:
                    ...
                    logger.debug(f"[PrioritizationMatrixTool] LLM retornou PrioritizationMatrix válida na tentativa {attempt + 1}")
                    return matrix
                else:
                    logger.warning(f"[PrioritizationMatrixTool] LLM retornou None na tentativa {attempt + 1}")
                    
            except ValidationError as e:
                logger.warning(f"[PrioritizationMatrixTool] ValidationError na tentativa {attempt + 1}: {e}")
    ...
    def _validate_prioritization_matrix(self, matrix: PrioritizationMatrix) -> None:
        ...
        if matrix.total_items >= 3:
            ...
            if critical_ratio > 0.5:
                logger.warning(f"Distribuição de prioridades pode estar inflacionada: {critical_ratio:.1%} CRITICAL")
            
            if low_ratio > 0.5:
                logger.warning(f"Distribuição de prioridades pode estar inadequada: {low_ratio:.1%} LOW")
        ...
        if sorted(ranks) != expected_ranks:
            raise ValueError(f"Ranks devem ser únicos e sequenciais 1..{matrix.total_items}, encontrado: {sorted(ranks)}")
        
        logger.info(f"PrioritizationMatrix validada - {matrix.total_items} items, Balanceada: {matrix.is_balanced()}")
    ...
    def get_quality_metrics(self, matrix: PrioritizationMatrix) -> dict:
        ...
        return {
            ...
            "critical_ratio": matrix.critical_count / matrix.total_items if matrix.total_items > 0 else 0,

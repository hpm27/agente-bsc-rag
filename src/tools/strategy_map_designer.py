...
class StrategyMapDesignerTool:
    ...
    async def _retrieve_bsc_knowledge(
        ...
        # Construir query RAG focada em Strategy Map
        company_name = diagnostic.company_info.name if diagnostic.company_info else "Empresa"
        sector = diagnostic.company_info.sector if diagnostic.company_info else "Geral"
...
- Principais gaps: {', '.join(diagnostic.top_gaps[:3]) if diagnostic.top_gaps else 'N/A'}
...
"""
        ...
        prompt_content = DESIGN_STRATEGY_MAP_PROMPT.format(
            ...
            tools_results=tools_summary,
            rag_context=rag_context
        )
        
        messages = [
            SystemMessage(content="Você é um consultor BSC sênior especialista em Strategy Maps (Kaplan & Norton framework)."),
        ...
        try:
            ...
            if strategy_map is None:
                ...
                raise ValueError("LLM structured output retornou None - possível finish_reason != 'stop'")
            
            logger.info(f"[StrategyMapDesigner] Strategy Map extraído: 4 perspectivas completas")
    ...
    async def _map_cause_effect_connections(
        self,
        perspectives_dict: Dict[str, StrategyMapPerspective]
        ...
        messages = [
            SystemMessage(content="Você é um consultor BSC especialista em mapeamento causa-efeito."),
            HumanMessage(content=prompt_content)
        ]
        ...
        try:
            ...
            if result is None or not result.connections:
                logger.warning("[StrategyMapDesigner] LLM não retornou conexões causa-efeito (usando mínimo)")
                # Criar conexões mínimas default (4 obrigatórias)
                return self._create_default_connections(perspectives_dict)
            
            logger.info(f"[StrategyMapDesigner] Conexões causa-efeito mapeadas: {len(result.connections)}")
    ...
    def _create_default_connections(
        ...
        if learning_obj and process_obj and learning_obj.objectives and process_obj.objectives:
            connections.append(CauseEffectConnection(
    ...
    async def design_strategy_map(
        ...
        try:
            ...
            rag_context = await self._retrieve_bsc_knowledge(diagnostic, tools_results)
            ...
            logger.info(f"[StrategyMapDesigner] Strategy Map completo: 4 perspectivas, "

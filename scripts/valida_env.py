...
def main():
    ...
    if not default_model:
        ...
        if default_model.startswith("claude-"):
            ...
            if not is_valid:
                errors.append(f"[ERRO] ANTHROPIC_API_KEY necessaria para modelo {default_model}")
            else:
                ok_messages.append(msg)
    ...
    if vector_store_type not in ["qdrant", "weaviate", "redis"]:
        ...
        if vector_store_type == "qdrant":
            ...
            warnings.append("[WARN] Lembre-se de iniciar Qdrant: docker-compose up -d qdrant")

        elif vector_store_type == "weaviate":
            host = os.getenv("WEAVIATE_HOST", "localhost")
            port = os.getenv("WEAVIATE_PORT", "8080")
            ok_messages.append(f"[OK] Weaviate: {host}:{port}")
            warnings.append("[WARN] Lembre-se de iniciar Weaviate: docker-compose up -d weaviate")

        elif vector_store_type == "redis":
            host = os.getenv("REDIS_HOST", "localhost")
            port = os.getenv("REDIS_PORT", "6379")
            ok_messages.append(f"[OK] Redis: {host}:{port}")
            warnings.append("[WARN] Lembre-se de iniciar Redis: docker-compose up -d redis")
    ...
    if cache_enabled:
        ...
        warnings.append("[WARN] Embedding cache DESATIVADO (vai processar embeddings toda vez)")
        ...
        if contextual_provider not in ["openai", "anthropic"]:
            ...
            if contextual_provider == "anthropic":
                ...
                if not anthropic_key or anthropic_key.startswith("your_"):
                    errors.append(
                        "[ERRO] ANTHROPIC_API_KEY necessaria para " "CONTEXTUAL_PROVIDER=anthropic"
                    )
    else:
    ...
    if literature_dir.exists():
        ...
        if docs:
            ok_messages.append(f"[OK] {len(docs)} documentos BSC encontrados em {literature_dir}")
        else:
            warnings.append(f"[WARN] Nenhum documento BSC encontrado em {literature_dir}")

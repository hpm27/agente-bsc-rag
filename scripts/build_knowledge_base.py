...
def generate_metadata_from_content(content: str, filename: str) -> dict[str, Any]:
    """
...
def save_metadata_to_index(directory: str, filename: str, metadata: dict[str, Any]) -> bool:
    """
    ...
    try:
        ...
        for doc in existing_docs:
            if doc.get("filename") == filename:
                logger.info(
                    f"      [AUTO-METADATA] Documento {filename} já existe no index.json (preservando manual)"
                )
                return False
...
def main():
    ...
    logger.info(
        f"   [OK] Embeddings: {embedding_manager.provider} ({embedding_manager.model_name if embedding_manager.provider == 'openai' else 'fine-tuned'})"
    )

    chunker = TableAwareChunker()
    logger.info("   [OK] Chunker: TableAwareChunker")
    ...
    if settings.enable_contextual_retrieval and settings.anthropic_api_key:
        ...
        logger.info("   [OK] Contextual Retrieval: Habilitado (Anthropic)")
    else:
        logger.info("   [WARN]  Contextual Retrieval: Desabilitado")

    # Cria/recria índice
    logger.info(f"\n[SETUP] Criando índice '{settings.vector_store_index}'...")
    try:
        vector_store.create_index(index_name=settings.vector_store_index, force_recreate=True)
    ...
    for doc_file in tqdm(all_files, desc="Documentos"):
        ...
        if doc_file.suffix == ".pdf":
            ...
            with open(doc_file, encoding="utf-8") as f:
                content = f.read()
                doc = {"content": content, "source": doc_file.name, "num_pages": 1}

        if not doc:
            logger.error("      [ERRO] Erro ao carregar")
            continue

        # Divide em chunks (com ou sem Contextual Retrieval)
        if contextual_chunker:
            logger.info("      [CONTEXT] Aplicando Contextual Retrieval...")
            contextual_chunks = contextual_chunker.chunk_text(
                text=doc["content"],
                metadata={"source": doc["source"], "num_pages": doc["num_pages"]},
            )
            ...
            base_chunks = chunker.chunk_text(
                doc["content"], metadata={"source": doc["source"], "num_pages": doc["num_pages"]}
            )
        ...
        if not doc_metadata and settings.enable_auto_metadata_generation:
            ...
            logger.info("      [AUTO-METADATA] Gerando metadados automaticamente...")

            # Gerar metadados a partir do conteúdo
            generated_metadata = generate_metadata_from_content(
                content=doc["content"], filename=doc_filename
            )
    ...
    for i, (chunk, embedding) in enumerate(zip(all_chunks, embeddings)):
        ...
        metadata = {k: v for k, v in chunk.items() if k not in ["content", "source", "page"]}
    ...
    try:
        ...
        for i in range(0, total_docs, batch_size):
            ...
            vector_store.add_documents(documents=batch_docs, embeddings=batch_embeddings)
            logger.info(
                f"   [BATCH] {i+len(batch_docs)}/{total_docs} documentos adicionados ({int((i+len(batch_docs))/total_docs*100)}%)"
            )
    ...
    try:
        ...
        logger.info("[STATS] Estatisticas:")
    ...
    try:
        ...
        results = vector_store.vector_search(query_embedding=test_embedding, k=3)

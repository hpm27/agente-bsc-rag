...
def run_example_queries():
    ...
    for i, example in enumerate(example_queries, 1):
        ...
        try:
            # Executar workflow
            result = workflow.run(query=example["query"], session_id=f"example-{i}")

            # Exibir resultado
            print_result(result)

            results.append({"query": example["query"], "success": True, "result": result})

        except Exception as e:
            logger.error(f"[ERRO] Falha ao processar query: {e}")
            results.append({"query": example["query"], "success": False, "error": str(e)})
...
def run_interactive_mode():
    ...
    while True:
        try:
            ...
            result = workflow.run(query=query, session_id=f"{session_id}-{query_count}")

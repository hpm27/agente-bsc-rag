...
def sample_profiles():
    ...
    for i in range(3):
        ...
        profile.metadata["approval_status"] = "PENDING"  # approval_status está em metadata, não engagement
...
def test_export_clients_list_has_correct_columns(csv_exporter, sample_profiles):
    ...
    expected_columns = [
        "client_id", "company_name", "sector", "size",
        "current_phase", "approval_status",
        "created_at", "updated_at",
        "total_challenges", "total_objectives"
    ]
...
def test_export_recommendations_success(csv_exporter, sample_diagnostic_with_recs, sample_profiles):
    ...
    csv_path = csv_exporter.export_recommendations(
        sample_diagnostic_with_recs, sample_profiles[0]
    )
...
def test_export_recommendations_priority_high_first(csv_exporter, sample_diagnostic_with_recs, sample_profiles):

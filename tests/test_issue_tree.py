...
def mock_agents():
    ...
    financial = Mock()
    financial.invoke = Mock(
        return_value={"response": "Perspectiva Financeira BSC: Lucratividade e ROI"}
    )

    customer = Mock()
    customer.invoke = Mock(
        return_value={"response": "Perspectiva Clientes BSC: Satisfacao e retencao"}
    )

    process = Mock()
    process.invoke = Mock(
        return_value={"response": "Perspectiva Processos BSC: Eficiencia operacional"}
    )

    learning = Mock()
    learning.invoke = Mock(
        return_value={"response": "Perspectiva Aprendizado BSC: Capacitacao equipe"}
    )
...
def test_facilitate_issue_tree_basic_workflow(
    mock_llm, mock_agents, valid_company_info, valid_strategic_context
):
...
def test_facilitate_issue_tree_max_depth_3(
    ...
    mock_synthesis = SolutionPathsOutput(
        solution_paths=["Solucao 1", "Solucao 2", "Solucao 3", "Solucao 4"],
...
def test_issue_node_creation_valid():
    ...
    node = IssueNode(
        text="Baixa lucratividade empresa SaaS B2B", level=0, parent_id=None, is_leaf=False
    )
...
def test_issue_tree_analysis_validator_root_node_missing():
    ...
    with pytest.raises(ValueError, match="Arvore deve ter exatamente 1 root node"):
        IssueTreeAnalysis(
            root_problem="Problema raiz", nodes=[child1, child2], max_depth=1  # Sem root (level=0)
        )

...
class TestE2EWorkflow:
    ...
    def test_comparative_query(self, workflow):
        ...
        perspectives_lower = [
            p.lower() if isinstance(p, str) else str(p).lower() for p in perspectives_covered
        ]
...
class TestPerformanceOptimizations:
    ...
    def test_multilingual_search_pt_br_query(self, workflow):
        ...
        assert (
            len(confident_agents) > 0
        ), "Busca multilíngue deve recuperar respostas com boa confiança"
    ...
    def test_parallel_agent_execution(self, workflow):
        ...
        if num_perspectives >= 3:
            assert (
                execution_time < 200
            ), f"Workflow muito lento: {execution_time:.2f}s (esperado <200s)"
...
class TestMetrics:
    ...
    def test_latency_percentiles(self, workflow):
        ...
        logger.info(
            f"[TEST METRICS] Executando {len(sample_queries)} queries para medir latência..."
        )
    ...
    def test_judge_approval_rate(self, workflow):
        ...
        logger.info(
            f"[TEST METRICS] Executando {len(sample_queries)} queries para medir approval rate..."
        )
...
class TestSystemReadiness:
    ...
    def test_dataset_indexed(self):
        ...
        try:
            ...
            assert (
                num_vectors >= 1000
            ), f"Dataset muito pequeno: {num_vectors} chunks (esperado >=1000)"

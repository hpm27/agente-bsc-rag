...
def query_decomposer(mock_llm):
    ...
    return QueryDecomposer(llm=mock_llm, enabled=True, min_query_length=50, score_threshold=2)
...
def test_decomposer_disabled_nunca_decompoe(mock_llm):
    ...
    decomposer = QueryDecomposer(llm=mock_llm, enabled=False)  # DESABILITADO
...
def test_threshold_customizado(mock_llm):
    ...
    decomposer_high = QueryDecomposer(
        llm=mock_llm, enabled=True, min_query_length=50, score_threshold=8  # MUITO ALTO
    )
...
TOTAL: 20 TESTES (15+ requerido [OK])

...
"""
    ...
    @pytest.mark.asyncio
    async def test_facilitate_cot_consulting_with_fallback(
        self, 
        orchestrator, 
        mock_client_profile
    ):
        ...
        with patch('langchain_openai.ChatOpenAI') as mock_chat:
            ...
            result = await orchestrator.facilitate_cot_consulting(
                client_profile=mock_client_profile,
                client_query="Como implementar BSC?"
            )
    ...
    @pytest.mark.asyncio
    async def test_facilitate_cot_consulting_error_logging(
        self, 
        orchestrator, 
        mock_client_profile
    ):

...
def test_list_metrics_returns_200(client, override_auth, mock_api_key, sample_metrics):
    ...
    try:
        ...
        response = client.get("/api/v1/metrics", headers={"X-API-Key": mock_api_key})
...
def test_list_metrics_validates_limit(client, override_auth, mock_api_key):
    ...
    try:
        ...
        response = client.get(
            "/api/v1/metrics", headers={"X-API-Key": mock_api_key}, params={"limit": 0}  # Inválido
        )
...
def test_get_stats_returns_200(client, override_auth, mock_api_key):
    ...
    # Mock performance service
    mock_service = Mock()
    mock_service.aggregate_stats = Mock(
        return_value={
            "total_requests": 100,
            "error_requests": 5,
            "error_rate": 5.0,
    ...
    try:
        ...
        response = client.get("/api/v1/metrics/stats", headers={"X-API-Key": mock_api_key})

...
def test_create_notification_returns_notification_id(
    notification_service, valid_notification, mock_mem0_client
):
...
def test_mark_as_read_returns_true_when_found(notification_service, mock_mem0_client):
    ...
    mock_mem0_client.search = Mock(
        return_value=[
            {
                "id": "mem0_id_123",
                "metadata": {"notification_id": "notif_test_123", "status": "unread"},
            }
        ]
    )

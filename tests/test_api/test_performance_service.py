...
import pytest
from api.services.performance_service import LLM_PRICING, PerformanceService
...
def test_get_metrics_returns_filtered_list(performance_service):
    ...
    # Mock Mem0 get_all
    performance_service.client.get_all = Mock(
        return_value={
            "results": [
                {
                    "metadata": {
    ...
    metrics = performance_service.get_metrics(endpoint="/api/v1/diagnostics", limit=100)

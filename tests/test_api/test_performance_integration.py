...
def test_llm_tokens_captured_in_context_vars(override_auth, mock_api_key):
    ...
    """
    from api.middleware.performance import llm_tokens_in_ctx, llm_tokens_out_ctx, llm_model_name_ctx, track_llm_tokens
    ...
    print(f"[OK] LLM tokens capturados e acumulados: in={llm_tokens_in_ctx.get()} out={llm_tokens_out_ctx.get()} model={model_name}")
...
def test_performance_system_end_to_end_smoke(override_auth, mock_api_key):
    ...
    assert hasattr(diagnostic_agent, 'track_llm_tokens'), "DiagnosticAgent nao importou track_llm_tokens"
    assert hasattr(onboarding_agent, 'track_llm_tokens'), "OnboardingAgent nao importou track_llm_tokens"

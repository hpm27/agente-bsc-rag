...
def test_api_key_invalid(client: TestClient):
    ...
    response = client.get("/api/v1/clients", headers={"X-API-Key": INVALID_API_KEY})

    assert response.status_code == 401
    assert (
        "inválida" in response.json()["detail"].lower()
...
def test_create_client_success(client: TestClient, auth_headers: dict):
    ...
    response = client.post("/api/v1/clients", headers=auth_headers, json=payload)
    ...
    if response.status_code == 201:
        ...
        assert (
            "erro" in response.json().get("detail", "").lower()
            or "error" in response.json().get("detail", "").lower()
        )
...
def test_list_clients(client: TestClient, auth_headers: dict):
    ...
    response = client.get(
        "/api/v1/clients", headers=auth_headers, params={"page": 1, "page_size": 10}
    )
...
def test_execute_swot_no_client(client: TestClient, auth_headers: dict):
    ...
    response = client.post("/api/v1/tools/swot", headers=auth_headers, json=payload)
...
def test_register_webhook_success(client: TestClient, auth_headers: dict):
    ...
    response = client.post("/api/v1/webhooks", headers=auth_headers, json=payload)

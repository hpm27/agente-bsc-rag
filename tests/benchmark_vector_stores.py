...
class VectorDBBenchmark:
    ...
    def get_embeddings(self, texts: list[str]) -> list[list[float]]:
        ...
        response = self.openai_client.embeddings.create(
            model="text-embedding-3-large", input=texts, dimensions=self.embedding_dim
        )
    ...
    def benchmark_qdrant(self) -> BenchmarkResult:
        ...
        for query_emb in query_embeddings:
            start = time.time()
            self.qdrant.search(collection_name=collection_name, query_vector=query_emb, limit=10)
    ...
    def benchmark_weaviate(self) -> BenchmarkResult:
        ...
        with collection.batch.dynamic() as batch:
            for i, doc in enumerate(self.documents):
                batch.add_object(
                    properties={"text": doc["text"], "doc_id": doc["id"], **doc["metadata"]},
                    vector=embeddings[i],
                )
        indexing_time = time.time() - start_time
        docs_per_sec = len(self.documents) / indexing_time
        ...
        for query_emb in query_embeddings:
            start = time.time()
            collection.query.near_vector(near_vector=query_emb, limit=10)
            latencies.append(time.time() - start)
    ...
    def benchmark_redis(self) -> BenchmarkResult:
        ...
        self.redis_client.ft(index_name).create_index(
            fields=schema, definition=IndexDefinition(prefix=["doc:"], index_type=IndexType.HASH)
        )
    ...
    def _calculate_recall_qdrant(
        self, collection_name: str, query_embeddings: list[list[float]]
    ) -> float:
        """Calcula Recall@10 para Qdrant."""
        recalls = []
        for i, query_emb in enumerate(query_embeddings):
            results = self.qdrant.search(
                collection_name=collection_name, query_vector=query_emb, limit=10
            )
    ...
    def _calculate_recall_weaviate(
        self, collection: Any, query_embeddings: list[list[float]]
    ) -> float:
        """Calcula Recall@10 para Weaviate."""
        recalls = []
        for i, query_emb in enumerate(query_embeddings):
            results = collection.query.near_vector(near_vector=query_emb, limit=10)
    ...
    def _calculate_recall_redis(
        self, index_name: str, query_embeddings: list[list[float]]
    ) -> float:
    ...
    def save_results(
        self, results: list[BenchmarkResult], output_path: str = "tests/benchmark_results.json"
    ):

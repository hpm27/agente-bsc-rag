...
def mock_retriever():
    ...
    retriever.retrieve_with_decomposition = Mock(
        return_value=[
            SearchResult(
...
def test_decomposition_strategy_complex_query(mock_retriever):
    ...
    mock_retriever.retrieve_with_decomposition = AsyncMock(
        return_value=[
            SearchResult(
...
def test_hybrid_search_strategy_multilingual(mock_retriever):
    ...
    mock_retriever.retrieve.assert_called_with(
        query=query, k=10, use_hybrid=True, use_rerank=True, multilingual=True, filters=None
    )

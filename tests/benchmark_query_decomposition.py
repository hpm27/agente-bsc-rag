...
class BenchmarkExecutor:
    ...
    async def run_benchmark(self):
        ...
        for i, query_data in enumerate(queries, 1):
            ...
            # Baseline
            print(f"  [BASELINE] Executando retrieval normal...")
            baseline_result = await self.run_baseline(query_data)
            self.baseline_results.append(baseline_result)
            print(f"    Latência: {baseline_result['latency']:.2f}s | Recall@10: {baseline_result['recall_at_10']:.2%} | Precision@5: {baseline_result['precision_at_5']:.2%}")
            
            # Decomposition
            print(f"  [DECOMPOSITION] Executando retrieval com decomposição...")
            decomp_result = await self.run_decomposition(query_data)
            self.decomposition_results.append(decomp_result)
            print(f"    Latência: {decomp_result['latency']:.2f}s | Recall@10: {decomp_result['recall_at_10']:.2%} | Precision@5: {decomp_result['precision_at_5']:.2%}")
            print(f"    Sub-queries: {decomp_result['num_subqueries']} | Heurística: {decomp_result['heuristic_correct']}")
            
            print()
        
        print(f"{'='*80}")
        print(f"BENCHMARK COMPLETO - Gerando relatório...")
        print(f"{'='*80}\n")
    ...
    def generate_report(self):
        ...
        heuristic_accuracy = sum(1 for d in self.heuristic_decisions if d["correct"]) / len(self.heuristic_decisions)
        
        total_subqueries = sum(r.get("num_subqueries", 0) for r in self.decomposition_results)
        avg_subqueries_when_decomposed = (
            mean([r["num_subqueries"] for r in self.decomposition_results if r["num_subqueries"] > 0])
            if any(r["num_subqueries"] > 0 for r in self.decomposition_results) else 0
        )
        
        # Improvements
        recall_improvement = ((mean(decomp_recalls) - mean(baseline_recalls)) / mean(baseline_recalls) * 100) if mean(baseline_recalls) > 0 else 0
        precision_improvement = ((mean(decomp_precisions) - mean(baseline_precisions)) / mean(baseline_precisions) * 100) if mean(baseline_precisions) > 0 else 0
        latency_overhead = mean(decomp_latencies) - mean(baseline_latencies)
        
        # Median improvement (com proteção contra divisão por zero)
        median_recall_improvement = ((median(decomp_recalls) - median(baseline_recalls)) / median(baseline_recalls) * 100) if median(baseline_recalls) > 0 else 0
        median_precision_improvement = ((median(decomp_precisions) - median(baseline_precisions)) / median(baseline_precisions) * 100) if median(baseline_precisions) > 0 else 0
...
|----------|--------|------|--------|
| **Recall@10 Improvement** | > +30% | {recall_improvement:+.1f}% | {'✅ PASS' if recall_improvement > 30 else '❌ FAIL'} |
| **Precision@5 Improvement** | > +25% | {precision_improvement:+.1f}% | {'✅ PASS' if precision_improvement > 25 else '❌ FAIL'} |
| **Latência Adicional** | < 3s | +{latency_overhead:.2f}s | {'✅ PASS' if latency_overhead < 3.0 else '❌ FAIL'} |
| **Heurística Accuracy** | > 80% | {heuristic_accuracy:.1%} | {'✅ PASS' if heuristic_accuracy > 0.80 else '❌ FAIL'} |
...
"""
        ...
        for baseline, decomp in zip(self.baseline_results, self.decomposition_results):
            ...
            category = next((q["category"] for q in self.load_queries() if q["id"] == query_id), "unknown")
...
**Recall@10:**
- {'✅ Query Decomposition melhorou recall em ' + f'{recall_improvement:.1f}%' if recall_improvement > 0 else '❌ Baseline teve melhor recall'}
- {'✅ Target de +30% foi ATINGIDO' if recall_improvement > 30 else '❌ Target de +30% NÃO foi atingido'}

**Precision@5:**
- {'✅ Query Decomposition melhorou precision em ' + f'{precision_improvement:.1f}%' if precision_improvement > 0 else '❌ Baseline teve melhor precision'}
- {'✅ Target de +25% foi ATINGIDO' if precision_improvement > 25 else '❌ Target de +25% NÃO foi atingido'}

**Latência:**
- Overhead médio: +{latency_overhead:.2f}s
- {'✅ Dentro do target de <3s' if latency_overhead < 3.0 else '❌ Acima do target de 3s'}

**Heurística:**
- Accuracy: {heuristic_accuracy:.1%}
- {'✅ Target de >80% foi ATINGIDO' if heuristic_accuracy > 0.80 else '❌ Target de >80% NÃO foi atingido'}

**Recomendação Final:**
"""
        
        # Decisão GO/NO-GO
        if (recall_improvement > 30 and precision_improvement > 25 and 
            latency_overhead < 3.0 and heuristic_accuracy > 0.80):
            report += """
✅ **GO** - Query Decomposition VALIDADA para produção!
...
"""
        ...
        print(f"{'='*80}")
        print(f"Recall@10 Improvement:    {recall_improvement:+.1f}% {'[OK]' if recall_improvement > 30 else '[FAIL]'}")
        print(f"Precision@5 Improvement:  {precision_improvement:+.1f}% {'[OK]' if precision_improvement > 25 else '[FAIL]'}")
        print(f"Latência Adicional:       +{latency_overhead:.2f}s {'[OK]' if latency_overhead < 3.0 else '[FAIL]'}")
        print(f"Heurística Accuracy:      {heuristic_accuracy:.1%} {'[OK]' if heuristic_accuracy > 0.80 else '[FAIL]'}")
        print(f"{'='*80}\n")

...
from src.memory.schemas import ClientProfile, CompanyInfo, StrategicContext, EngagementState
...
def test_stats_summary_calculates_metrics_correctly(mock_summaries):
    ...
    # Arrange
    from app.components.dashboard import _render_stats_summary
...
def test_render_filters_returns_all_when_no_filters(mock_summaries):
    ...
    # Arrange
    from app.components.dashboard import _render_filters
...
def test_filter_by_search_query_works(mock_summaries):
    ...
    filtered = [
        s for s in mock_summaries
        if search_lower in s.get('company_name', '').lower()
    ]

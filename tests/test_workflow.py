...
class TestBSCWorkflow:
    ...
    def test_analyze_query_node(self, workflow):
        ...
        state = BSCState(query="Quais são os principais KPIs da perspectiva financeira?")
    ...
    def test_workflow_run_simple_query(self, workflow):
        ...
        result = workflow.run(query="O que é Balanced Scorecard?", session_id="test-session-001")
    ...
    def test_workflow_run_specific_perspective(self, workflow):
        ...
        result = workflow.run(query="Como melhorar o ROI da empresa?")
    ...
    def test_workflow_run_multiple_perspectives(self, workflow):
        ...
        result = workflow.run(query="Como a satisfação do cliente impacta a lucratividade?")
    ...
    def test_workflow_with_chat_history(self, workflow):
        ...
        result = workflow.run(query="Quais são as 4 perspectivas?", chat_history=chat_history)
    ...
    def test_decide_next_step_approved(self, workflow):
        ...
        state = BSCState(
            ...
            judge_evaluation=JudgeEvaluation(
                approved=True, score=0.9, feedback="Excelente resposta"
            ),
        )
    ...
    def test_decide_next_step_needs_refinement(self, workflow):
        ...
        state = BSCState(
            query="Teste",
            judge_evaluation=JudgeEvaluation(
                approved=False, score=0.6, feedback="Precisa melhorar"
            ),
    ...
    def test_decide_next_step_max_iterations(self, workflow):
        ...
        state = BSCState(
            query="Teste",
            judge_evaluation=JudgeEvaluation(
                approved=False, score=0.6, feedback="Ainda precisa melhorar"
            ),
            needs_refinement=True,
    ...
    def test_finalize_node(self, workflow):
        ...
        state = BSCState(
            ...
            judge_evaluation=JudgeEvaluation(approved=True, score=0.95, feedback="Aprovado"),
            refinement_iteration=0,
        )
...
if __name__ == "__main__":
    ...
    try:
        ...
        result = workflow.run(query="O que é Balanced Scorecard?", session_id="test-001")

...
def valid_strategic_objective_customer():
    ...
    return StrategicObjective(
        ...
        timeframe="6 meses",
        success_criteria=["NPS >= 50 pontos até Q2 2026", "Churn rate < 5% ao ano"],
...
def valid_strategy_map_perspective_financial(valid_strategic_objective_financial):
    ...
    return StrategyMapPerspective(name="Financeira", objectives=[obj1, obj2])
...
def valid_strategy_map(
    ...
    customer_perspective = StrategyMapPerspective(
        ...
        objectives=[
            ...
            StrategicObjective(
                ...
                timeframe="12 meses",
                success_criteria=[
                    "Churn anual menor ou igual a 5%",
                    "NPS acima de 50 pontos em Q4/2026",
                ],
            ),
        ],
    )
    ...
    process_perspective = StrategyMapPerspective(
        ...
        objectives=[
            ...
            StrategicObjective(
                ...
                timeframe="9 meses",
                success_criteria=[
                    "OEE >= 85% nas linhas críticas",
                    "Disponibilidade >= 90% (redução paradas não programadas)",
                ],
            ),
        ],
    )
    ...
    learning_perspective = StrategyMapPerspective(
        ...
        objectives=[
            ...
            StrategicObjective(
                ...
                timeframe="12 meses",
                success_criteria=[
                    "Mínimo 50 kaizens implementados no ano",
                    "Participação de 70% ou mais da equipe no programa",
                ],
            ),
        ],
    )
    ...
    return StrategyMap(
        ...
        strategic_priorities=["Excelência Operacional", "Customer Intimacy", "Inovação de Produto"],
...
def test_strategy_map_perspective_min_objectives_violated():
    ...
    obj = StrategicObjective(
        ...
        timeframe="12 meses",
        success_criteria=[
            "Critério de sucesso específico número um",
            "Critério de sucesso específico número dois",
        ],
    )

    with pytest.raises(ValidationError) as exc_info:
        StrategyMapPerspective(name="Financeira", objectives=[obj])  # Apenas 1 objective (mínimo 2)
...
def test_strategy_map_perspective_max_objectives_violated():
    ...
    with pytest.raises(ValidationError) as exc_info:
        StrategyMapPerspective(name="Financeira", objectives=objectives)
...
def test_strategy_map_min_connections_violated():
    ...
    financial = StrategyMapPerspective(
        ...
        objectives=[
            StrategicObjective(
                ...
                timeframe="12 meses",
                success_criteria=[
                    "Critério sucesso válido número 1",
                    "Critério sucesso válido número 2",
                ],
            ),
            StrategicObjective(
                name="Objetivo Financeiro 2",
                description="Desc" * 20,
                perspective="Financeira",
                target_date=datetime(2026, 12, 31),
                owner="CFO",
    ...
    connections = [
        CauseEffectConnection(
            source_objective_id="l1",
            target_objective_id="p1",
            relationship_type="enables",
            rationale="R" * 50,
        ),
        CauseEffectConnection(
            source_objective_id="p1",
            target_objective_id="c1",
            relationship_type="drives",
            rationale="R" * 50,
        ),
        CauseEffectConnection(
            source_objective_id="c1",
            target_objective_id="f1",
            relationship_type="drives",
            rationale="R" * 50,
        ),
    ]
    ...
    with pytest.raises(ValidationError):
        StrategyMap(
            ...
            strategic_priorities=[
                ...
                "Priority 2",
                "Priority 3",
                "Priority 4",  # 4 priorities (máximo 3)
            ],
        )
...
def test_alignment_report_score_bounds():
    ...
    # Score válido (0)
    report_min = AlignmentReport(score=0, is_balanced=False, validation_checks={"check1": False})
    assert report_min.score == 0

    # Score válido (100)
    report_max = AlignmentReport(score=100, is_balanced=True, validation_checks={"check1": True})
    assert report_max.score == 100

    # Score inválido (<0)
    with pytest.raises(ValidationError):
        AlignmentReport(score=-1, is_balanced=False, validation_checks={"check1": False})

    # Score inválido (>100)
    with pytest.raises(ValidationError):
        AlignmentReport(score=101, is_balanced=True, validation_checks={"check1": True})
...
def test_alignment_report_critical_gaps_count(valid_alignment_report):
    ...
    report_no_gaps = AlignmentReport(
        score=100, is_balanced=True, gaps=[], validation_checks={"check1": True}  # Sem gaps
    )

...
def valid_client_profile_1():
    ...
    return ClientProfile.model_construct(
        ...
        context=StrategicContext(
            mission="Inovação tecnológica", vision="Líder em transformação digital"
        ),
...
def valid_client_profile_2():
    ...
    return ClientProfile.model_construct(
        ...
        context=StrategicContext(mission="Excelência financeira", vision="Banco digital top 3"),
...
def valid_client_profile_3_archived():
    ...
    return ClientProfile.model_construct(
        ...
        company=CompanyInfo(name="OldCorp Ltda", sector="Manufatura", size="pequena"),
        context=StrategicContext(),
        engagement=EngagementState(current_phase="COMPLETED"),
...
def test_list_all_profiles_success_search_method(
    mem0_client, valid_client_profile_1, valid_client_profile_2
):
...
def test_list_all_profiles_fallback_get_all(mem0_client, valid_client_profile_1):
    ...
    mem0_client.client.get_all.return_value = [
        {"metadata": {"profile_data": valid_client_profile_1.to_mem0(), "user_id": "client_001"}}
    ]
...
def test_list_all_profiles_filters_archived(
    mem0_client, valid_client_profile_1, valid_client_profile_3_archived
):
...
def test_list_all_profiles_includes_archived_when_requested(
    mem0_client, valid_client_profile_1, valid_client_profile_3_archived
):
...
def test_list_all_profiles_sorted_by_updated_at_desc(
    mem0_client, valid_client_profile_1, valid_client_profile_2
):
...
def test_get_client_summary_success(mem0_client, valid_client_profile_1):
    ...
    mem0_client.client.get_all.return_value = [
        {"metadata": {"swot_analysis_data": {}, "kpi_framework_data": {}}}
    ]
...
def test_get_client_summary_with_approval_status(mem0_client, valid_client_profile_1):
    ...
    # Arrange
    valid_client_profile_1.metadata["approval_status"] = (
        "APPROVED"  # approval_status está em profile.metadata
    )
    mem0_client.load_profile = MagicMock(return_value=valid_client_profile_1)
    mem0_client.client.get_all.return_value = []
...
def test_integration_list_and_summarize(
    mem0_client, valid_client_profile_1, valid_client_profile_2
):
    ...
    mem0_client.client.search.return_value = [
        {"metadata": {"profile_data": valid_client_profile_1.to_mem0(), "user_id": "client_001"}},
        {"metadata": {"profile_data": valid_client_profile_2.to_mem0(), "user_id": "client_002"}},
    ]
    mem0_client.load_profile = MagicMock(
        side_effect=[valid_client_profile_1, valid_client_profile_2]
    )
    mem0_client.client.get_all.return_value = []

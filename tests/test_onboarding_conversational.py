...
async def test_collect_client_info_first_turn_initialization(
    ...
    result = await onboarding_agent.collect_client_info(
        "user_123", "Sou da TechCorp, setor de tecnologia", valid_bsc_state
    )
...
async def test_collect_client_info_accumulation_multiple_turns(
    ...
    result1 = await onboarding_agent.collect_client_info(
        "user_123", "Sou da TechCorp, tecnologia", valid_bsc_state
    )
...
async def test_collect_client_info_minimum_info_completion(
    ...
    result = await onboarding_agent.collect_client_info(
        "user_123", "OK, pode prosseguir", valid_bsc_state
    )
...
async def test_collect_client_info_challenges_accumulation(
    ...
    await onboarding_agent.collect_client_info(
        "user_123", "Temos falta de capital e crescimento lento", valid_bsc_state
    )
...
async def test_collect_client_info_llm_error_fallback_question(
    ...
    result = await onboarding_agent.collect_client_info("user_123", "Olá", valid_bsc_state)

    # Deve retornar pergunta de fallback
    assert result["is_complete"] is False
    assert "question" in result
    assert (
        "nome da sua empresa" in result["question"].lower()
        or "qual o nome" in result["question"].lower()
    )
...
def test_prompts_are_anti_hallucination():
    ...
    extraction_has_anti_hallucination = (
        "NÃO invente" in ENTITY_EXTRACTION_SYSTEM or "NÃO adivinhe" in ENTITY_EXTRACTION_SYSTEM
    )
    ...
    context_aware_has_anti_hallucination = "NÃO repita" in CONTEXT_AWARE_QUESTION_SYSTEM
...
async def test_validate_challenge_correct(onboarding_agent):
    ...
    result = await onboarding_agent._validate_extraction(
        "Baixa satisfação de clientes", "challenge"
    )
...
async def test_validate_objective_correct(onboarding_agent):
    ...
    mock_response = MagicMock()
    mock_response.content = json.dumps(
        {"classified_as": "objective", "confidence": 1.0, "reasoning": "Meta futura quantificada"}
    )
    onboarding_agent.llm.ainvoke = AsyncMock(return_value=mock_response)

    result = await onboarding_agent._validate_extraction(
        "Aumentar satisfação para 90%", "objective"
    )
...
async def test_validate_challenge_misclassified_as_objective(onboarding_agent):
    ...
    mock_response = MagicMock()
    mock_response.content = json.dumps(
        {"classified_as": "objective", "confidence": 0.9, "reasoning": "Meta futura identificada"}
    )
    onboarding_agent.llm.ainvoke = AsyncMock(return_value=mock_response)

    result = await onboarding_agent._validate_extraction("Reduzir custos em 15%", "challenge")
...
async def test_validate_objective_misclassified_as_challenge(onboarding_agent):
    ...
    result = await onboarding_agent._validate_extraction("Processos ineficientes", "objective")
...
async def test_validate_ambiguous_text(onboarding_agent):
    ...
    result = await onboarding_agent._validate_extraction("Queremos crescer", "objective")
...
async def test_validate_high_confidence(onboarding_agent):
    ...
    result = await onboarding_agent._validate_extraction(
        "Reduzir custos operacionais em 15%", "objective"
    )
...
async def test_validate_low_confidence(onboarding_agent):
    ...
    mock_response = MagicMock()
    mock_response.content = json.dumps(
        {"classified_as": "ambiguous", "confidence": 0.3, "reasoning": "Extremamente vago"}
    )
    onboarding_agent.llm.ainvoke = AsyncMock(return_value=mock_response)

    result = await onboarding_agent._validate_extraction("Melhorar desempenho", "objective")
...
async def test_validate_json_parse_error_fallback(onboarding_agent):
    ...
    result = await onboarding_agent._validate_extraction("Baixa satisfação", "challenge")
...
```"""
    ...
    result = await onboarding_agent._validate_extraction(
        "Alta rotatividade de funcionários", "challenge"
    )
...
async def test_validate_batch_mixed(onboarding_agent):
    ...
    responses = [
        ...
        MagicMock(
            content=json.dumps(
                {"classified_as": "challenge", "confidence": 0.95, "reasoning": "Problema atual"}
            )
        ),
        # Objective correto
        MagicMock(
            content=json.dumps(
                {"classified_as": "objective", "confidence": 1.0, "reasoning": "Meta quantificada"}
            )
        ),
        # Ambíguo
        MagicMock(
            content=json.dumps(
                {"classified_as": "ambiguous", "confidence": 0.4, "reasoning": "Vago"}
            )
        ),
    ]
...
async def test_extract_with_validation_reclassify_challenge_to_objective(onboarding_agent):
    ...
    mock_validation.content = json.dumps(
        {"classified_as": "objective", "confidence": 0.95, "reasoning": "Meta quantificada"}
    )
...
async def test_extract_with_validation_reclassify_objective_to_challenge(onboarding_agent):
    ...
    mock_validation = MagicMock()
    mock_validation.content = json.dumps(
        {"classified_as": "challenge", "confidence": 0.9, "reasoning": "Problema atual"}
    )

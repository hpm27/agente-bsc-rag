...
@pytest.mark.skip(
    reason="Benchmark comparativo não faz parte da suíte unitária CI; instável em paralelo"
)
...
def print_comparison(result_threadpool: dict, result_asyncio: dict):
    ...
    print(
        f"{'Tempo Total (s)':<30} {result_threadpool['total_time']:<20.3f} {result_asyncio['total_time']:<20.3f}"
    )
    print(
        f"{'Soma Individual (s)':<30} {result_threadpool['sum_individual']:<20.3f} {result_asyncio['sum_individual']:<20.3f}"
    )
    print(
        f"{'Speedup':<30} {result_threadpool['speedup']:<20.2f}x {result_asyncio['speedup']:<20.2f}x"
    )
    print(
        f"{'Num Agentes':<30} {result_threadpool['num_agents']:<20} {result_asyncio['num_agents']:<20}"
    )

    # Calcular ganho de AsyncIO sobre ThreadPool
    if result_threadpool["total_time"] > 0:
        improvement = (
            (result_threadpool["total_time"] - result_asyncio["total_time"])
            / result_threadpool["total_time"]
        ) * 100
    ...
    all_agents = set(result_threadpool["agent_times"].keys()) | set(
        result_asyncio["agent_times"].keys()
    )

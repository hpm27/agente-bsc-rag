...
def test_mmr_basic_functionality(reranker, sample_documents, sample_embeddings):
    ...
    diverse_docs = reranker.rerank_with_diversity(
        query=query, documents=sample_documents, embeddings=sample_embeddings, top_n=3
    )
...
def test_mmr_diversity_vs_relevance(reranker, sample_documents):
    ...
    embeddings_similar = embeddings_similar / np.linalg.norm(
        embeddings_similar, axis=1, keepdims=True
    )
...
def test_mmr_empty_documents(reranker):
    ...
    diverse_docs = reranker.rerank_with_diversity(
        query="teste", documents=[], embeddings=np.array([]).reshape(0, 384), top_n=5
    )
...
def test_mmr_embeddings_mismatch(reranker, sample_documents):
    ...
    diverse_docs = reranker.rerank_with_diversity(
        query="teste", documents=sample_documents, embeddings=wrong_embeddings, top_n=3
    )
...
def test_mmr_adaptive_topn_integration(reranker, sample_documents, sample_embeddings):
    ...
    # Query complexa
    query_complex = (
        "Como implementar BSC considerando perspectivas financeira, clientes e processos?"
    )
...
def test_mmr_with_unnormalized_embeddings(reranker, sample_documents):
    ...
    unnormalized_embeddings = (
        np.random.randn(5, 384) * 10
    )  # Multiplicar por 10 para garantir norma alta

    # Verificar que embeddings não estão normalizados
    assert np.linalg.norm(unnormalized_embeddings[0]) > 1.1

    diverse_docs = reranker.rerank_with_diversity(
        query="teste", documents=sample_documents, embeddings=unnormalized_embeddings, top_n=3
    )
...
def test_mmr_single_document(reranker):
    ...
    single_doc = [{"content": "Teste", "metadata": {"source": "test.pdf"}, "rerank_score": 0.9}]
    single_embedding = np.random.randn(1, 384)
    single_embedding = single_embedding / np.linalg.norm(single_embedding)

    diverse_docs = reranker.rerank_with_diversity(
        query="teste", documents=single_doc, embeddings=single_embedding, top_n=5
    )
...
def test_rerank_basic(reranker, sample_documents):
    ...
    with patch.object(reranker.client, "rerank") as mock_rerank:
        ...
        reranked = reranker.rerank(
            query="Como implementar BSC?", documents=sample_documents, top_n=3
        )
...
def test_rerank_adaptive_multilingual(reranker, sample_documents):
    ...
    with patch.object(reranker.client, "rerank") as mock_rerank:
        ...
        # Query em português
        reranked = reranker.rerank(
            query="O que é BSC?", documents=sample_documents, top_n=2, adaptive_multilingual=True
        )
...
def test_rerank_error_handling(reranker, sample_documents):
    ...
    with patch.object(reranker.client, "rerank") as mock_rerank:
        ...
        reranked = reranker.rerank(query="teste", documents=sample_documents, top_n=3)
...
def test_rerank_with_scores_basic(reranker, sample_documents):
    ...
    with patch.object(reranker.client, "rerank") as mock_rerank:
        ...
        filtered = reranker.rerank_with_scores(
            query="teste", documents=sample_documents, threshold=0.5
        )
...
def test_rerank_with_scores_high_threshold(reranker, sample_documents):
    ...
    with patch.object(reranker.client, "rerank") as mock_rerank:
        ...
        filtered = reranker.rerank_with_scores(
            query="teste", documents=sample_documents, threshold=0.9
        )
...
def test_hybrid_reranker_basic():
    ...
    with patch("src.rag.reranker.CohereReranker"):
        ...
        hybrid.fusion_reranker.fuse = Mock(
            return_value=[
        ...
        hybrid.cohere_reranker.rerank = Mock(
            return_value=[{"content": "Final 1", "source": "book1.pdf", "rerank_score": 0.9}]
        )
        ...
        final = hybrid.rerank(
            query="teste", vector_results=vector_results, text_results=text_results, top_n=1
        )

...
def test_company_info_minimal():
    ...
    company = CompanyInfo(name="Empresa Teste", sector="Serviços")
...
def test_company_info_invalid_founded_year():
    ...
    with pytest.raises(ValidationError):
        CompanyInfo(name="Empresa", sector="Tecnologia", founded_year=1500)  # Antes de 1800

    with pytest.raises(ValidationError):
        CompanyInfo(name="Empresa", sector="Tecnologia", founded_year=2030)  # Futuro
...
def test_client_profile_round_trip_complex():
    ...
    swot = SWOTAnalysis(strengths=["Forte equipe"], weaknesses=["Processos manuais"])
    diagnostic = DiagnosticData(
        swot=swot, pain_points=["Falta de métricas"], key_findings=["BSC necessário"]
    )
    company = CompanyInfo(
        name="TechCorp Brasil", sector="Tecnologia", size="média", founded_year=2015
    )
    context = StrategicContext(mission="Inovar sempre", core_values=["Inovação", "Excelência"])
    engagement = EngagementState(current_phase="DISCOVERY", progress_percentage=50)
...
def test_client_profile_metadata_custom():
    ...
    profile = ClientProfile(
        company=company,
        metadata={"custom_field_1": "value1", "custom_field_2": 123, "nested": {"key": "value"}},
    )

...
def mock_agents():
    ...
    for agent_name in ["financial_agent", "customer_agent", "process_agent", "learning_agent"]:
        agent = MagicMock()
        agent.retrieve_async = AsyncMock(
            return_value=[
                MagicMock(
                    page_content=f"Conhecimento BSC da perspectiva {agent_name} sobre implementação de ações estratégicas..."
                )
            ]
        )
...
@pytest.mark.asyncio
async def test_facilitate_action_plan_success(
    mock_llm, mock_agents, sample_client_profile, sample_diagnostic_result
):
...
async def test_facilitate_action_plan_without_diagnostic(mock_llm, sample_client_profile):
    ...
    action_plan = await tool.facilitate(
        client_profile=sample_client_profile, diagnostic_results=None
    )
...
async def test_facilitate_action_plan_validation_error(mock_llm, sample_client_profile):
    ...
    with pytest.raises(ValidationError):
        await tool.facilitate(client_profile=sample_client_profile, max_retries=1)
...
async def test_facilitate_action_plan_retry_logic(mock_llm, sample_client_profile):
    ...
    action_plan = await tool.facilitate(client_profile=sample_client_profile, max_retries=2)
...
async def test_synthesize_action_plan_success(mock_llm, sample_client_profile):
    ...
    action_plan = await tool.synthesize(
        individual_actions=individual_actions, client_profile=sample_client_profile
    )
...
async def test_build_bsc_knowledge_context_agent_error(mock_llm):
    ...
    context = await tool._build_bsc_knowledge_context(financial_agent=failing_agent)
...
@pytest.mark.asyncio
async def test_e2e_action_plan_generation(
    mock_llm, mock_agents, sample_client_profile, sample_diagnostic_result
):
...
async def test_e2e_action_plan_with_diagnostic_agent(sample_client_profile):
    ...
    async def _call_llm_with_retry_and_timeout(
        tool, client_profile, max_retries=3, timeout_per_attempt=90
    ):
        """
        ...
        for attempt in range(1, max_retries + 1):
            ...
            try:
                print(
                    f"[E2E] Tentativa {attempt}/{max_retries} - Iniciando chamada LLM...",
                    flush=True,
                )
                ...
                if attempt < max_retries:
                    ...
                    pytest.fail(
                        f"Teste E2E falhou após {max_retries} tentativas (timeout {timeout_per_attempt}s/tentativa)"
                    )
    ...
    print("[E2E] Configuração LLM:", flush=True)
    print(f"  - Model: {settings.diagnostic_llm_model}", flush=True)
    print("  - max_completion_tokens: 1500 (3 ações mínimas)", flush=True)
    print("  - request_timeout: 90s/tentativa", flush=True)
    print("  - Retry: 3 tentativas com exponential backoff", flush=True)
    print("  - Latência esperada: 60-120s (NORMAL para E2E com LLM real)", flush=True)
    ...
    print("[E2E] Validando ActionPlan...", flush=True)
    ...
    assert (
        action_plan.total_actions >= 3
    ), f"Esperado >= 3 ações, obtido {action_plan.total_actions}"
    ...
    for i, action in enumerate(action_plan.action_items):
        ...
        assert action.priority in [
            ...
            "MEDIUM",
            "LOW",
        ], f"Ação {i}: priority inválida: {action.priority}"
        assert action.effort in [
            "HIGH",
            "MEDIUM",
            "LOW",
        ], f"Ação {i}: effort inválido: {action.effort}"
    ...
    assert (
        len(perspectives_used) >= 2
    ), f"Esperado >= 2 perspectivas, obtido {len(perspectives_used)}: {perspectives_used}"
    ...
    print("[E2E] TESTE PASSOU! ActionPlan validado:", flush=True)

...
Info válida para benchmarking.
    ...
    """
    return CompanyInfo(
        name="TechCorp Brasil Ltda",  # > 2 chars ✅
        sector="Tecnologia",  # obrigatório ✅
        size="média",  # Literal válido ✅
        industry="Software as a Service (SaaS)",  # opcional fornecido ✅
        region="Brasil",  # opcional fornecido ✅
        founded_year=2018  # 1800-2025 ✅
    )
...
def valid_diagnostic_four_perspectives() -> dict[str, DiagnosticResult]:
    ...
    return {
        "Financeira": DiagnosticResult(
            ...
            current_state="Receita recorrente crescendo 25% ao ano mas margem EBITDA de apenas 18% devido custos operacionais elevados",  # > 20 chars ✅
            gaps=["Falta visibilidade de custos por produto", "Pricing subotimizado"],  # min 1 ✅
            opportunities=["Implementar ABC Costing", "Revisar pricing strategy"],  # min 1 ✅
            priority="HIGH",
            key_insights=["Margem 7pp abaixo do mercado SaaS"]
        ),
        "Clientes": DiagnosticResult(
            perspective="Clientes",
            current_state="NPS de 42 pontos está 15 pontos abaixo do benchmark setorial, indicando insatisfação com onboarding manual",  # > 20 chars ✅
        ...
        "Processos Internos": DiagnosticResult(
            ...
            current_state="Lead Time de 14 dias está 40% acima do benchmark (10 dias) devido processos manuais em fulfillment",  # > 20 chars ✅
        ...
        "Aprendizado e Crescimento": DiagnosticResult(
            ...
            current_state="Retenção de talentos de 85% ao ano está no mercado mas falta programa estruturado de desenvolvimento",  # > 20 chars ✅
            gaps=["Sem programa de desenvolvimento estruturado", "Treinamento ad-hoc"],
            opportunities=["Criar programa de desenvolvimento", "Implementar IDP (Individual Development Plan)"],
            priority="LOW",
...
def valid_kpi_framework() -> KPIFramework:
    ...
    return KPIFramework(
        financial_kpis=[
            KPIDefinition(
                name="Margem EBITDA Anual",  # min_length=10 ✅
                perspective="Financeira",
                description="Margem EBITDA anual calculada como EBITDA dividido por Receita Total, expressa em percentual",  # > 50 chars ✅
                metric_type="custo",  # obrigatório ✅
                target_value="25%",
                measurement_frequency="mensal",  # lowercase ✅
                data_source="Demonstrações Financeiras (ERP)"  # obrigatório, > 5 chars ✅
            ),
            KPIDefinition(
                name="ROI - Return on Investment Total",  # min_length=10 ✅
                perspective="Financeira",
                description="Return on Investment calculado como (Ganho do Investimento - Custo do Investimento) / Custo do Investimento",  # > 50 chars ✅
                metric_type="quantidade",  # obrigatório ✅
                target_value="20%",
                measurement_frequency="trimestral",  # lowercase ✅
                data_source="Relatórios Financeiros Trimestrais"  # obrigatório ✅
            )
        ],
        customer_kpis=[
            KPIDefinition(
                name="NPS - Net Promoter Score Clientes",  # min_length=10 ✅
                perspective="Clientes",
                description="Net Promoter Score medindo lealdade dos clientes através da probabilidade de recomendação da empresa",  # > 50 chars ✅
                metric_type="qualidade",  # obrigatório ✅
                target_value="60",
                measurement_frequency="trimestral",  # lowercase ✅
                data_source="Pesquisa NPS Automatizada (CRM)"  # obrigatório ✅
            ),
            KPIDefinition(
                name="Churn Rate Mensal Clientes",  # min_length=10 ✅
                perspective="Clientes",
                description="Taxa de churn mensal calculada como percentual de clientes perdidos em relação à base total de clientes ativos",  # > 50 chars ✅
                metric_type="quantidade",  # obrigatório ✅
                target_value="3%",
                measurement_frequency="mensal",  # lowercase ✅
                data_source="Sistema de Gestão de Clientes (CRM)"  # obrigatório ✅
            )
        ],
        process_kpis=[
            KPIDefinition(
                name="Lead Time Fulfillment Médio",  # min_length=10 ✅
                perspective="Processos Internos",
                description="Lead Time médio de fulfillment calculado como tempo total desde recebimento do pedido até entrega ao cliente",  # > 50 chars ✅
                metric_type="tempo",  # obrigatório ✅
                target_value="10",
                measurement_frequency="mensal",  # lowercase ✅
                data_source="Sistema ERP - Módulo de Operações"  # obrigatório ✅
            ),
            KPIDefinition(
                name="Cycle Time Processos Críticos",  # min_length=10 ✅
                perspective="Processos Internos",
                description="Cycle Time médio dos processos críticos medindo tempo total de execução de início ao fim de cada processo",  # > 50 chars ✅
                metric_type="tempo",  # obrigatório ✅
                target_value="24",
                measurement_frequency="semanal",  # lowercase ✅
                data_source="Plataforma BPM (Business Process Management)"  # obrigatório ✅
            )
        ],
        learning_kpis=[
            KPIDefinition(
                name="Retenção de Talentos Anual",  # min_length=10 ✅
                perspective="Aprendizado e Crescimento",
                description="Taxa de retenção anual calculada como percentual de colaboradores que permaneceram na empresa durante o ano completo",  # > 50 chars ✅
                metric_type="quantidade",  # obrigatório ✅
                target_value="90%",
                measurement_frequency="anual",  # lowercase ✅
                data_source="Sistema RH - Turnover Analytics"  # obrigatório ✅
            ),
            KPIDefinition(
                name="Horas Treinamento por Funcionário Ano",  # min_length=10 ✅
                perspective="Aprendizado e Crescimento",
                description="Horas médias de treinamento por funcionário ao ano, incluindo treinamentos técnicos, comportamentais e estratégicos",  # > 50 chars ✅
                metric_type="quantidade",  # obrigatório ✅
                target_value="80",
                measurement_frequency="anual",  # lowercase ✅
                data_source="LMS (Learning Management System) + RH"  # obrigatório ✅
            )
        ]
    )
...
def valid_benchmark_comparison() -> BenchmarkComparison:
    ...
    return BenchmarkComparison(
        ...
        gap_type="negative",  # gap > 5 → negative ✅
        benchmark_source="Setor Tecnologia SaaS Brasil 2024 (média empresas médio porte B2B - ARR $5-20M)",  # > 20 chars, específico ✅
        insight="Margem EBITDA 7pp abaixo do mercado, indicando custos operacionais elevados ou pricing subotimizado. Empresas similares alcançam 25% via automação de processos e pricing value-based.",  # > 50 chars ✅
        priority="HIGH"
    )
...
def valid_benchmark_report_8_comparisons() -> BenchmarkReport:
    ...
    return BenchmarkReport(
        comparisons=[
            ...
            BenchmarkComparison(
                ...
                BenchmarkComparison(
                    ...
                    company_value="15%",
                    benchmark_value="21%",
                    gap=6.0,  # > 5 → gap_type="negative" válido ✅
                    gap_type="negative",
            ...
            BenchmarkComparison(
                ...
                benchmark_value="88%",
                gap=3.0,
                gap_type="neutral",  # gap < 5 → neutral ✅
        ...
        overall_performance="abaixo_mercado",  # Maioria gaps negativos ✅
        priority_gaps=[
            "Margem EBITDA 7pp abaixo do benchmark setorial (Financeira) - impacto: 30% do potencial de lucro não realizado",  # > 30 chars ✅
            "NPS 15 pontos abaixo do benchmark (Clientes) - risco: churn 20% maior que mercado",  # > 30 chars ✅
            "Lead Time 40% maior que benchmark (Processos Internos) - impacto: perda de 15% oportunidades por lentidão"  # > 30 chars ✅
        ],
        recommendations=[
...
def test_benchmark_comparison_source_too_generic():
    ...
    with pytest.raises(ValidationError) as exc_info:
        BenchmarkComparison(
            ...
            gap=6.0,
            gap_type="negative",
            benchmark_source="mercado tech empresas médias",  # 30 chars mas termo genérico → ValidationError ✅
            insight="ROI 6pp abaixo do benchmark indicando retorno sobre investimento menor",
            priority="MEDIUM"
        )
    
    assert "benchmark_source 'mercado tech empresas médias' parece muito genérico" in str(exc_info.value)
    ...
    with pytest.raises(ValidationError) as exc_info:
        BenchmarkReport(
            comparisons=[
                ...
                BenchmarkComparison(perspective="Financeira", metric_name="EBITDA", company_value="18%", benchmark_value="25%", gap=7.0, gap_type="negative", benchmark_source="Setor Tech SaaS Brasil 2024 (médio porte B2B)", insight="Margem EBITDA 7pp abaixo indicando custos operacionais elevados", priority="HIGH"),
                BenchmarkComparison(perspective="Financeira", metric_name="ROI", company_value="15%", benchmark_value="21%", gap=6.0, gap_type="negative", benchmark_source="Benchmark SaaS Global 2024 (B2B médio porte)", insight="ROI 6pp abaixo indicando retorno menor que concorrentes", priority="MEDIUM"),
                BenchmarkComparison(perspective="Financeira", metric_name="CAC", company_value="$500", benchmark_value="$400", gap=25.0, gap_type="negative", benchmark_source="CAC SaaS Brasil 2024 (empresas médio porte B2B)", insight="CAC 25% acima do benchmark setorial indicando custos de aquisição elevados", priority="LOW"),
                BenchmarkComparison(perspective="Financeira", metric_name="LTV", company_value="$5k", benchmark_value="$7k", gap=28.0, gap_type="negative", benchmark_source="LTV SaaS Brasil 2024 (empresas B2B médio porte)", insight="LTV 28% abaixo do mercado indicando necessidade de retenção melhor", priority="MEDIUM"),
                BenchmarkComparison(perspective="Financeira", metric_name="ARR", company_value="$2M", benchmark_value="$3M", gap=33.0, gap_type="negative", benchmark_source="ARR SaaS Brasil 2024 (empresas B2B médio porte)", insight="ARR 33% abaixo do benchmark indicando crescimento de receita menor", priority="LOW"),
                # 2 Clientes
                BenchmarkComparison(perspective="Clientes", metric_name="NPS", company_value="42", benchmark_value="57", gap=15.0, gap_type="negative", benchmark_source="NPS B2B SaaS Brasil 2024 (empresas médio porte B2B)", insight="NPS 15 pontos abaixo do benchmark indicando insatisfação clientes", priority="HIGH"),
                BenchmarkComparison(perspective="Clientes", metric_name="CSAT", company_value="70%", benchmark_value="85%", gap=15.0, gap_type="negative", benchmark_source="CSAT SaaS Brasil 2024 (empresas médio porte)", insight="CSAT abaixo do benchmark setorial indicando satisfação menor", priority="MEDIUM"),
                # 2 Processos
                BenchmarkComparison(perspective="Processos Internos", metric_name="Lead Time", company_value="14d", benchmark_value="10d", gap=40.0, gap_type="negative", benchmark_source="Lead Time SaaS Brasil 2024 (médio porte)", insight="Lead Time alto indicando processos lentos e ineficientes", priority="MEDIUM"),
                BenchmarkComparison(perspective="Processos Internos", metric_name="Cycle Time", company_value="36h", benchmark_value="24h", gap=50.0, gap_type="negative", benchmark_source="Cycle Time Tech 2024 (empresas B2B médio porte)", insight="Cycle Time alto indicando necessidade de automação processos", priority="LOW"),
                # 0 Aprendizado e Crescimento → ValidationError! ❌
            ],
            overall_performance="abaixo_mercado",
...
def test_generate_benchmarks_missing_company_info(
    mock_llm,
    valid_diagnostic_four_perspectives
):
    ...
    with pytest.raises(ValueError) as exc_info:
        tool.generate_benchmarks(
            company_info=None,  # None → ValidationError ✅
            diagnostic=valid_diagnostic_four_perspectives,
            kpi_framework=None,
            use_rag=False
        )
...
def test_generate_benchmarks_incomplete_diagnostic(
    mock_llm,
    valid_company_info
):

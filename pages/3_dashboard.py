...
with st.spinner("Carregando dados do BSC da sessao atual..."):
    # CORREÇÃO SESSAO 43: load_strategy_map agora retorna 3 valores (objectives, connections, error)
    objectives, connections, objectives_error = load_strategy_map(user_id)
...
with col1:
    ...
    if st.button("Ver Strategy Map Detalhado", type="primary", width="stretch"):
        st.switch_page("pages/1_strategy_map.py")

with col2:
    # CORREÇÃO SESSAO 43: Substituir use_container_width (deprecated)
    if st.button("Ver Action Plan Completo", type="primary", width="stretch"):

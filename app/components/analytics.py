...
def _make_api_request(endpoint: str, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
...
def render_analytics_dashboard() -> None:
    ...
    with col2:
        endpoint_filter = st.selectbox(
            "Endpoint",
            options=["Todos"] + _get_all_endpoints(),
            index=0,
...
def _render_traffic_chart(period: str, endpoint: Optional[str] = None) -> None:
    ...
    # Gráfico de linha
    st.line_chart(
        df.set_index("timestamp")[["count", "errors"]],
...
def _render_performance_chart(period: str) -> None:
    ...
    with st.spinner("Carregando métricas de performance..."):
        endpoints_data = _make_api_request("/analytics/endpoints", params={
            "metric": "latency",
            "limit": 10,
            "period": period
        })
    ...
    endpoints_with_latency = [
        e for e in endpoints_data
        if e.get("latency_p50") is not None
    ]
    ...
    # Gráfico de barras
    st.bar_chart(
        df.set_index("endpoint")[["P50", "P95", "P99"]],
        use_container_width=True
    )
...
def _render_errors_chart(period: str) -> None:
    ...
    df = pd.DataFrame([
        ...
        for e in sorted(endpoints_with_errors, key=lambda x: x.get("error_rate", 0), reverse=True)[:10]
    ])
    
    # Gráfico de barras
    st.bar_chart(
        df.set_index("endpoint")[["Taxa de Erro (%)"]],
        use_container_width=True
    )
...
def _render_consumers_table(period: str) -> None:
    ...
    with st.spinner("Carregando dados de consumers..."):
        consumers_data = _make_api_request("/analytics/consumers", params={
            "limit": 10,
            "period": period
        })
    ...
    df = pd.DataFrame([
        {
            ...
            "Latência P50 (ms)": f"{e.get('latency_p50', 0):.0f}" if e.get("latency_p50") else "N/A",
            "Latência P95 (ms)": f"{e.get('latency_p95', 0):.0f}" if e.get("latency_p95") else "N/A",
            "Latência P99 (ms)": f"{e.get('latency_p99', 0):.0f}" if e.get("latency_p99") else "N/A"
        }
...
def _get_all_endpoints() -> List[str]:
    ...
    try:
        endpoints_data = _make_api_request("/analytics/endpoints", params={"limit": 100, "period": "24h"})
        if endpoints_data:
